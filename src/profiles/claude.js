...
function onAddRulesProfile(targetDir, assetsDir) {
	...
	if (fs.existsSync(sourceFile)) {
		try {
			...
			log('debug', `[Claude] Created Task Master instructions at ${taskMasterClaudeFile}`)
			...
			if (fs.existsSync(userClaudeFile)) {
				...
				if (!content.includes(importLine)) {
					...
					log('info', `[Claude] Task Master import already present in ${userClaudeFile}`)
				}
			} else {
			...
			log('error', `[Claude] Failed to set up Claude instructions: ${err.message}`)
		}
	}
}
...
function onRemoveRulesProfile(targetDir) {
	...
	try {
		...
		if (fs.existsSync(userClaudeFile)) {
			...
			if (updatedContent === '# Claude Code Instructions' || updatedContent === '') {
				...
				fs.writeFileSync(userClaudeFile, updatedContent + '\n')
				log('debug', `[Claude] Removed Task Master import from ${userClaudeFile}`)
			}
		}
	} catch (err) {
		log('error', `[Claude] Failed to remove Claude instructions: ${err.message}`)
	}
}
...
function onPostConvertRulesProfile(targetDir, assetsDir) {
	...
	if (fs.existsSync(mcpConfigPath)) {
		try {
			...
			fs.writeFileSync(mcpConfigPath, JSON.stringify(claudeConfig, null, '\t') + '\n')
			log('debug', `[Claude] Transformed MCP configuration to Claude format at ${mcpConfigPath}`)
		} catch (err) {
			log('error', `[Claude] Failed to transform MCP configuration: ${err.message}`)
		}
	}
}
