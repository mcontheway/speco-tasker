...
function onAddRulesProfile(targetDir, assetsDir) {
	...
	if (fs.existsSync(sourceFile)) {
		try {
			...
			log('debug', `[Amp] Created Task Master instructions at ${taskMasterAgentFile}`)
			...
			if (fs.existsSync(userAgentFile)) {
				...
				if (!content.includes(importLine)) {
					...
					log('info', `[Amp] Task Master import already present in ${userAgentFile}`)
				}
			} else {
...
function onRemoveRulesProfile(targetDir) {
	...
	try {
		...
		if (config['amp.mcpServers'] && config['amp.mcpServers']['task-master-ai']) {
			...
			if (remainingKeys.length === 0) {
				...
				log('info', '[Amp] Removed TaskMaster from settings.json, preserved other configurations')
			}
		} else {
...
function onPostConvertRulesProfile(targetDir, assetsDir) {
	...
	try {
		...
		if (mcpConfig['amp.mcpServers']) {
			log('info', '[Amp] settings.json already in Amp format, skipping transformation')
			return
		}
