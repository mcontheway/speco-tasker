...
function onPostConvertRulesProfile(targetDir, assetsDir) {
	...
	try {
		...
		if (mcpConfig.$schema) {
			log('info', '[OpenCode] opencode.json already in OpenCode format, skipping transformation')
			return
		}
		...
		log('error', `[OpenCode] Failed to transform opencode.json: ${error.message}`)
	}
}
...
function onRemoveRulesProfile(targetDir) {
	...
	try {
		...
		if (config.mcp && config.mcp['taskmaster-ai']) {
			...
			const remainingKeys = Object.keys(config).filter((key) => key !== '$schema')
		...
		log('error', `[OpenCode] Failed to clean up opencode.json: ${error.message}`)
	}
}
