...
function onRemoveRulesProfile(targetDir) {
	...
	try {
		...
		if (config['context_servers'] && config['context_servers']['task-master-ai']) {
			...
			if (remainingKeys.length === 0) {
				...
				log('info', '[Zed] Removed TaskMaster from settings.json, preserved other configurations')
			}
		} else {
...
function onPostConvertRulesProfile(targetDir, assetsDir) {
	...
	try {
		...
		if (mcpConfig['context_servers']) {
			log('info', '[Zed] settings.json already in Zed format, skipping transformation')
			return
		}
		...
		fs.writeFileSync(mcpConfigPath, JSON.stringify(zedConfig, null, '\t') + '\n')
