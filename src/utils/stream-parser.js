...
class StreamParserConfig {
	constructor(config = {}) {
		...
		this.itemValidator = config.itemValidator || StreamParserConfig.defaultItemValidator
	...
	validate() {
		...
		if (this.fallbackItemExtractor && typeof this.fallbackItemExtractor !== 'function') {
	...
	static defaultItemValidator(item) {
		return item && item.title && typeof item.title === 'string' && item.title.trim()
	}
}
...
class FallbackParser {
	...
	_processNewItems(fallbackItems) {
		...
		const itemsToAdd = fallbackItems.slice(this.progressTracker.parsedItems.length)
		const newItems = []
...
class StreamParserOrchestrator {
	...
	async processStream(textStream) {
		const processor = new StreamProcessor((chunk) => {
			this.bufferValidator.validateChunk(this.progressTracker.accumulatedText, chunk)
	...
	const fallbackParser = new FallbackParser(
		{
			...config,
			expectedTotal,
			itemValidator: config.itemValidator || StreamParserConfig.defaultItemValidator
		},
		progressTracker
	)
