...
export function setupMCPConfiguration(projectRoot, mcpConfigPath) {
	...
	if (!mcpConfigPath) {
		log('debug', '[MCP Config] No mcpConfigPath provided, skipping MCP configuration setup')
		return
	}
	...
	if (fs.existsSync(mcpPath)) {
		log('info', 'MCP configuration file already exists, checking for existing task-master-ai...')
		try {
			...
			const hasMCPString = Object.values(mcpConfig.mcpServers).some(
				(server) =>
					...
					server.args.some((arg) => typeof arg === 'string' && arg.includes('task-master-ai'))
			)
			if (hasMCPString) {
				log(
					'info',
...
export function removeTaskMasterMCPConfiguration(projectRoot, mcpConfigPath) {
	...
	try {
		...
		if (!hasTaskMaster) {
			result.success = true
			result.removed = false
			log('debug', `[MCP Config] Task Master not found in MCP config: ${mcpPath}`)
			return result
		}
		...
		Object.keys(mcpConfig.mcpServers).forEach((serverName) => {
			...
			if (
				...
				log('debug', `[MCP Config] Removed server '${serverName}' containing task-master-ai`)
			}
		})
		...
		if (result.hasOtherServers) {
			...
			result.removed = true
			result.deleted = true
			log('info', `[MCP Config] Removed MCP config file (no other servers remaining): ${mcpPath}`)
		}
	} catch (error) {
		result.error = error.message
		log('error', `[MCP Config] Failed to remove Task Master from MCP config: ${error.message}`)
	}
