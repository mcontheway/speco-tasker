...
import { removeTaskMasterMCPConfiguration, setupMCPConfiguration } from './create-mcp-config.js'
...
function replaceToolReferences(content, conversionConfig) {
	...
	const toolReferencePattern = new RegExp(`\\b(${Object.keys(toolNames).join('|')})\\b`, 'g')
...
export function convertRuleToProfileRule(sourcePath, targetPath, profile) {
	...
	try {
		...
		// Transform content
		const transformedContent = transformRuleContent(content, conversionConfig, globalReplacements)
		...
		for (const sourceFile of sourceFiles) {
			...
			try {
				...
				if (!fs.existsSync(sourcePath)) {
					log('warn', `[Rule Transformer] Source file not found: ${sourcePath}, skipping`)
					continue
				}
	...
	try {
		...
		if (sourceFiles.length > 0) {
			...
			if (!fs.existsSync(profileDir)) {
				result.success = true
				result.skipped = true
				log('debug', `[Rule Transformer] Profile directory does not exist: ${profileDir}`)
				return result
			}
			...
			if (fs.existsSync(targetDir)) {
				...
				const taskMasterFiles = sourceFiles.map((sourceFile) => profile.fileMap[sourceFile])
				...
				for (const taskMasterFile of taskMasterFiles) {
					...
					if (fs.existsSync(filePath)) {
						try {
							...
							log('debug', `[Rule Transformer] Removed Task Master file: ${taskMasterFile}`)
						} catch (error) {
							log(
								'error',
				...
				const remainingFiles = allFilePaths.filter((file) => !taskMasterFiles.includes(file))
				...
				if (remainingFiles.length === 0) {
					...
					log('debug', `[Rule Transformer] Removed empty rules directory: ${targetDir}`)
		...
		if (profile.mcpConfig !== false) {
			try {
				result.mcpResult = removeTaskMasterMCPConfiguration(projectRoot, profile.mcpConfigPath)
		...
		if (fs.existsSync(profileDir)) {
			...
			if (remainingContents.length === 0 && profile.profileDir !== '.') {
				...
				try {
					...
					log('debug', `[Rule Transformer] Removed empty profile directory: ${profileDir}`)
				} catch (error) {
					log(
						'error',
