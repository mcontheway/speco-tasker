...
import { createBorder, createProgressHeader, createProgressRow } from './tracker-ui.js'
...
class PriorityManager {
	...
	normalize(priority) {
		...
		return CONSTANTS.VALID_PRIORITIES.includes(lowercased) ? lowercased : CONSTANTS.DEFAULT_PRIORITY
	}
...
class ParsePrdTracker extends BaseProgressTracker {
	...
	_updateProgressDisplay(taskNumber, title, normalizedPriority) {
		...
		createProgressRow(this.multibar, ` ${taskIdCentered} | ${priorityDisplay} | {title}`, {
			title: displayTitle
		})
