...
# Supports: AGENT.md, CLAUDE.md, GEMINI.md, and .github/copilot-instructions.md
...
update_agent_file() {
    ...
    if [ ! -f "$target_file" ]; then
        ...
        sed -i.bak "s/\[项目名称\]/$(basename $REPO_ROOT)/" "$temp_file"
        sed -i.bak "s/\[日期\]/$(date +%Y年%m月%d日)/" "$temp_file"
        sed -i.bak "s/\[从所有 PLAN.MD 文件中提取\]/- $NEW_LANG + $NEW_FRAMEWORK ($CURRENT_BRANCH)/" "$temp_file"
        
        # Add project structure based on type
        if [[ "$NEW_PROJECT_TYPE" == *"web"* ]]; then
            sed -i.bak "s|\[从所有 PLAN.MD 文件中提取的实际结构\]|backend/\nfrontend/\ntests/|" "$temp_file"
        else
            sed -i.bak "s|\[从所有 PLAN.MD 文件中提取的实际结构\]|src/\ntests/|" "$temp_file"
        fi
        ...
        sed -i.bak "s|\[仅适用于活跃技术的命令\]|$COMMANDS|" "$temp_file"
        
        # Add code style
        sed -i.bak "s|\[仅适用于使用语言的代码风格\]|$NEW_LANG: 遵循标准规范|" "$temp_file"
        
        # Add recent changes
        sed -i.bak "s|\[最近 3 个功能以及它们添加的内容\]|- $CURRENT_BRANCH: 添加了 $NEW_LANG + $NEW_FRAMEWORK|" "$temp_file"
...
changes_section = re.search(r'## 最近变化\n(.*?)(\n\n|$)', content, re.DOTALL)
if changes_section:
    changes = changes_section.group(1).strip().split('\n')
    changes.insert(0, f"- $CURRENT_BRANCH: 添加了 $NEW_LANG + $NEW_FRAMEWORK")
    # Keep only last 3
    changes = changes[:3]
    content = re.sub(r'(## 最近变化\n).*?(\n\n|$)', 
                    f'\\1{chr(10).join(changes)}\\2', content, flags=re.DOTALL)

# Update date
content = re.sub(r'最后更新：\d{4}年\d{2}月\d{2}日', 
                f'最后更新：{datetime.now().strftime("%Y年%m月%d日")}', content)
...
case "$AGENT_TYPE" in
    "agent")
        update_agent_file "$AGENT_FILE" "Agent"
        ;;
    "claude")
    ...
    "")
        ...
        [ -f "$AGENT_FILE" ] && update_agent_file "$AGENT_FILE" "Agent"
        [ -f "$CLAUDE_FILE" ] && update_agent_file "$CLAUDE_FILE" "Claude Code"
        [ -f "$GEMINI_FILE" ] && update_agent_file "$GEMINI_FILE" "Gemini CLI"
        [ -f "$COPILOT_FILE" ] && update_agent_file "$COPILOT_FILE" "GitHub Copilot"

        # If no files exist, create Agent.md by default
        if [ ! -f "$AGENT_FILE" ] && [ ! -f "$CLAUDE_FILE" ] && [ ! -f "$GEMINI_FILE" ] && [ ! -f "$COPILOT_FILE" ]; then
            echo "No agent context files found. Creating Agent.md context file by default."
            update_agent_file "$AGENT_FILE" "Agent"
        fi
        ;;
    *)
        echo "ERROR: Unknown agent type '$AGENT_TYPE'. Use: agent, claude, gemini, copilot, or leave empty for all."
        exit 1
        ;;
esac
echo ""
...
echo ""
echo "Usage: $0 [agent|claude|gemini|copilot]"
echo "  - No argument: Update all existing agent context files"
echo "  - agent: Update only AGENT.md"
