...
export async function updateTasksDirect(args, log, context = {}) {
	...
	try {
		...
		if (result && result.success && Array.isArray(result.updatedTasks)) {
			logWrapper.success(`Successfully updated ${result.updatedTasks.length} tasks.`)
			return {
				success: true,
				data: {
			...
			logWrapper.error('Core updateTasks function did not return a successful structure.')
			return {
				success: false,
				error: {
