...
export function registerScopeUpTool(server) {
	server.addTool({
		...
		parameters: z.object({
			id: z.string().describe('Comma-separated list of task IDs to scope up (e.g., "1,3,5")'),
			strength: z
				.string()
				.optional()
		...
		execute: withNormalizedProjectRoot(async (args, { log, session }) => {
			try {
				...
				return handleApiResult(result, log, 'Error scoping up task', undefined, args.projectRoot)
			} catch (error) {
