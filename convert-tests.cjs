...
files.forEach(filePath => {
  ...
  content = content.replace(/import\s*\{\s*jest\s*\}\s*from\s*['"]@jest\/globals['"]/g, "const { jest } = require('@jest/globals')")
  content = content.replace(/import\s*jest\s*from\s*['"]@jest\/globals['"]/g, "const jest = require('@jest/globals')")

  // Convert other imports (simplified for this example)
  content = content.replace(/import\s+([\w\s{},*]+)\s+from\s+['"]([^'"]+)['"]/g, (match, imports, module) => {
