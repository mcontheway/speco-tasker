...
import { RULES_ACTIONS, RULES_SETUP_ACTION } from '../../src/constants/rules-actions.js'
	...
	describe('CLI Flag Format Validation', () => {
		test('should detect camelCase flags correctly', () => {
			...
			const camelCaseFlags = args.filter(
				(arg) => arg.startsWith('--') && /[A-Z]/.test(arg) && !arg.includes('-[A-Z]')
			)
...
describe('rules command', () => {
	...
	test('should handle rules remove <profile> command', async () => {
		...
		await program.parseAsync(['rules', RULES_ACTIONS.REMOVE, 'roo', '--force'], {
			from: 'user'
		})
	...
	test(`should handle rules --${RULES_SETUP_ACTION} command`, async () => {
		...
		try {
			...
			expect(() => {
				...
				const setupOption = command.options.find((opt) => opt.long === `--${RULES_SETUP_ACTION}`)
	...
	test('should show error for invalid action', async () => {
		...
		expect(mockConsoleError).toHaveBeenCalledWith(
			expect.stringMatching(
				new RegExp(`For interactive setup, use: task-master rules --${RULES_SETUP_ACTION}`, 'i')
			)
		)
