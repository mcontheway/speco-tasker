...
const mockMoveTask = jest
	.fn()
	.mockImplementation(async (tasksPath, sourceId, destinationId, generateFiles, options) => {
...
const mockMoveTaskDirect = jest.fn().mockImplementation(async (args, log, context = {}) => {
...
const mockMoveTaskCrossTagDirect = jest.fn().mockImplementation(async (args, log, context = {}) => {
	// Validate required parameters
	if (!args.sourceIds) {
		return {
			success: false,
...
export { mockMoveTask, mockMoveTaskDirect, mockMoveTaskCrossTagDirect, mockRegisterMoveTaskTool }
