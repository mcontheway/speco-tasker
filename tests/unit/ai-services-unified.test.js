...
describe('Unified AI Services', () => {
	...
	describe('generateTextService', () => {
		test('should use main provider/model and succeed', async () => {
			...
			expect(mockGetParametersForRole).toHaveBeenCalledWith('main', fakeProjectRoot)
		...
		test('should fall back to fallback provider if main fails', async () => {
			...
			mockAnthropicProvider.generateText.mockRejectedValueOnce(mainError).mockResolvedValueOnce({
			...
			expect(mockGetParametersForRole).toHaveBeenCalledWith('main', explicitRoot)
			expect(mockGetParametersForRole).toHaveBeenCalledWith('fallback', explicitRoot)
		...
		test('should fall back to research provider if main and fallback fail', async () => {
			...
			expect(mockGetParametersForRole).toHaveBeenCalledWith('main', fakeProjectRoot)
			expect(mockGetParametersForRole).toHaveBeenCalledWith('fallback', fakeProjectRoot)
			expect(mockGetParametersForRole).toHaveBeenCalledWith('research', fakeProjectRoot)
		...
		test('should throw error if all providers in sequence fail', async () => {
			mockAnthropicProvider.generateText.mockRejectedValue(new Error('Anthropic failed'))
			mockPerplexityProvider.generateText.mockRejectedValue(new Error('Perplexity failed'))
		...
		test('should handle retryable errors correctly', async () => {
			...
			expect(mockLog).toHaveBeenCalledWith(
				'info',
				expect.stringContaining('Something went wrong on the provider side. Retrying')
			)
		})
