...
describe('OpenAIProvider', () => {
	...
	describe('validateOptionalParams', () => {
		it('should accept valid maxTokens values', () => {
			expect(() => provider.validateOptionalParams({ maxTokens: 1000 })).not.toThrow()
			expect(() => provider.validateOptionalParams({ maxTokens: 1 })).not.toThrow()
			expect(() => provider.validateOptionalParams({ maxTokens: '1000' })).not.toThrow()
		})
		...
		it('should reject invalid maxTokens values', () => {
			...
			expect(() => provider.validateOptionalParams({ maxTokens: Infinity })).toThrow(Error)
			expect(() => provider.validateOptionalParams({ maxTokens: 'invalid' })).toThrow(Error)
		})

		it('should accept valid temperature values', () => {
			expect(() => provider.validateOptionalParams({ temperature: 0 })).not.toThrow()
			expect(() => provider.validateOptionalParams({ temperature: 0.5 })).not.toThrow()
			expect(() => provider.validateOptionalParams({ temperature: 1 })).not.toThrow()
		})

		it('should reject invalid temperature values', () => {
			expect(() => provider.validateOptionalParams({ temperature: -0.1 })).toThrow(Error)
			expect(() => provider.validateOptionalParams({ temperature: 1.1 })).toThrow(Error)
		})
	})
