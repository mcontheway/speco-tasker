...
describe('GeminiCliProvider', () => {
	...
	describe('_extractSystemMessage', () => {
		...
		it('should add JSON enforcement with no existing system message', () => {
			...
			expect(result.messages).toEqual([{ role: 'user', content: 'Return JSON format' }])
		})
	})
	...
	describe('generateObject', () => {
		...
		it('should handle JSON parsing errors by attempting manual extraction', async () => {
			...
			jest
				.spyOn(Object.getPrototypeOf(Object.getPrototypeOf(provider)), 'generateObject')
			...
			expect(generateObject).toHaveBeenCalledWith({
				...
				system: expect.stringContaining('CRITICAL: You MUST respond with ONLY valid JSON'),
				maxTokens: undefined,
