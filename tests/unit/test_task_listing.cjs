...
describe("任务列表功能验证", () => {
	...
	describe("任务列表数据结构", () => {
		...
		it("应该支持标签化任务结构", () => {
			const taggedTasks = {
				master: {
					tasks: [{ id: 1, title: "主分支任务", status: "pending" }],
				},
				feature: {
					tasks: [{ id: 2, title: "功能分支任务", status: "pending" }],
				},
			};
	...
	describe("任务过滤功能", () => {
		...
		it("应该能够按状态过滤任务", () => {
			...
			expect(
				inProgressTasks.every((task) => task.status === "in-progress"),
			).toBe(true);
		});
	...
	describe("任务统计信息", () => {
		...
		it("应该正确计算任务状态统计", () => {
			...
			expect(
				statusStats.pending + statusStats["in-progress"] + statusStats.done,
			).toBe(5);
		});
	...
	describe("错误处理", () => {
		...
		it("应该处理无效的任务数据", () => {
			...
			const validTasks = invalidTasks.filter(
