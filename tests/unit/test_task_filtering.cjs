...
describe('任务过滤功能验证', () => {
	...
	describe('状态过滤', () => {
		...
		it('应该支持按多个状态过滤', () => {
			...
			const activeTasks = filterTasks(mockTasks, { statuses: ['in-progress', 'pending'] })
		...
		it('应该支持排除特定状态', () => {
			const filterTasks = (tasks, filters) => {
				return tasks.filter(task => {
					// 排除状态过滤
					if (filters.excludeStatuses && filters.excludeStatuses.includes(task.status)) {
						return false
					}
			...
			const notDoneTasks = filterTasks(mockTasks, { excludeStatuses: ['done'] })
			expect(notDoneTasks.length).toBe(3)
			expect(notDoneTasks.map(t => t.id)).toEqual([1, 3, 4])

			const notPendingTasks = filterTasks(mockTasks, { excludeStatuses: ['pending'] })
	...
	describe('优先级过滤', () => {
		...
		it('应该支持按最小优先级过滤', () => {
			...
			const mediumOrHigherTasks = filterTasks(mockTasks, { minPriority: 'medium' })
		...
		it('应该支持按优先级范围过滤', () => {
			...
			const mediumRangeTasks = filterTasks(mockTasks, { priorityRange: ['medium', 'high'] })
	...
	describe('标签过滤', () => {
		...
		it('应该支持按多个标签过滤（OR逻辑）', () => {
			...
			const frontendOrAuthTasks = filterTasks(mockTasks, { tags: ['frontend', 'auth'] })
		...
		it('应该支持按多个标签过滤（AND逻辑）', () => {
			const filterTasks = (tasks, filters) => {
				return tasks.filter(task => {
					...
					if (filters.allTags && filters.allTags.length > 0) {
						const hasAllTags = filters.allTags.every(tag =>
							task.tags && task.tags.includes(tag)
						)
						if (!hasAllTags) {
							return false
			...
			const backendAndAuthTasks = filterTasks(mockTasks, { allTags: ['backend', 'auth'] })
			expect(backendAndAuthTasks.length).toBe(1)
			expect(backendAndAuthTasks[0].id).toBe(3)

			const noMatchingTasks = filterTasks(mockTasks, { allTags: ['frontend', 'database'] })
		...
		it('应该支持排除特定标签', () => {
			...
			const nonFrontendTasks = filterTasks(mockTasks, { excludeTags: ['frontend'] })
	...
	describe('复合过滤', () => {
				...
				return tasks.filter(task => {
					...
					if (filters.andConditions) {
						for (const condition of filters.andConditions) {
							if (condition.type === 'status' && task.status !== condition.value) {
								matches = false
								break
							}
							if (condition.type === 'priority' && task.priority !== condition.value) {
								matches = false
								break
							}
							if (condition.type === 'tag' && (!task.tags || !task.tags.includes(condition.value))) {
								matches = false
								break
							}
						}
		...
		it('应该支持自定义过滤器函数', () => {
			...
			const customFilteredTasks = filterTasks(mockTasks, {
				customFilter: (task) => task.estimatedHours > 6 && task.status === 'pending'
			})
	...
	describe('过滤结果处理', () => {
		it('应该返回过滤结果的统计信息', () => {
			...
			const filteredResult = filterTasksWithStats(mockTasks, { status: 'pending' })
