...
describe("任务导入功能验证", () => {
	...
	describe("导入数据结构验证", () => {
		...
		it("应该验证导入数据的版本兼容性", () => {
			...
			versionTests.forEach(
				({ version, compatible, supportsSubtasks, supportsDependencies }) => {
					const importData = {
						metadata: { version },
						tasks: [],
					};
					...
					// 模拟版本兼容性检查
					const isCompatible =
						version.startsWith("1.") || version.startsWith("0.9");
  ...
  <task id="1">
    ...
    <priority>medium</priority>
		...
		it("应该处理导入数据的ID冲突解决", () => {
			...
			const noConflicts = importTasks.filter(
				(importTask) =>
					!existingTasks.some((existing) => existing.id === importTask.id),
			);
	...
	describe("导入文件操作验证", () => {
		...
		it("应该验证导入文件的完整性检查", () => {
			...
			// 验证有效数据的完整性
			const isValidData =
				validImportData.tasks.length === validImportData.metadata.totalTasks;
			expect(isValidData).toBe(true);

			// 验证无效数据的完整性
			const isInvalidData =
				invalidImportData.tasks.length ===
				invalidImportData.metadata.totalTasks;
	...
	describe("导入配置和选项验证", () => {
		...
		it("应该验证导入选项的有效性", () => {
			...
			expect(validResolutions).not.toContain(
				invalidOptions.idConflictResolution,
			);
		...
		it("应该支持不同的合并策略", () => {
			...
			expect(overwriteResult.find((task) => task.id === 1).status).toBe(
				"in-progress",
			);
