...
describe('PromptManager', () => {
	...
	const expectedTemplatesDir = path.join(__dirname, '..', '..', 'src', 'prompts')
	...
	describe('loadPrompt', () => {
		...
		it('should handle array iteration with {{#each}}', () => {
			...
			expect(result.userPrompt).toBe('Tasks:\n- 1: First task\n- 2: Second task\n')
		})
		...
		it('should handle JSON serialization with triple braces', () => {
			...
			expect(result.userPrompt).toBe(`Analyze these tasks: ${JSON.stringify(tasks, null, 2)}`)
		})
		...
		it('should use specified variant key over conditions', () => {
			...
			const result = promptManager.loadPrompt('variant-prompt', { useResearch: false }, 'research')
		...
		it('should handle special characters in templates', () => {
			...
			expect(result.systemPrompt).toBe('System with "quotes" and \'apostrophes\'')
