...
describe("任务验证功能验证", () => {
	...
	describe("任务数据完整性验证", () => {
		it("应该验证必需字段的存在性", () => {
			...
			expect(validateRequiredFields(invalidTask).missingFields).toContain(
				"title",
			);

			expect(validateRequiredFields(incompleteTask).isValid).toBe(false);
			expect(validateRequiredFields(incompleteTask).missingFields).toEqual([
				"id",
			]);
		});
		...
		it("应该验证字段的数据类型", () => {
			const validateDataTypes = (task) => {
				const typeValidators = {
					...
					dependencies: (value) =>
						Array.isArray(value) &&
						value.every((dep) => typeof dep === "number"),
				};
	...
	describe("批量验证功能", () => {
		it("应该批量验证多个任务", () => {
			const validateTasks = (tasks) => {
				...
				tasks.forEach((task, index) => {
					...
					if (task.id && typeof task.id !== "number")
						taskErrors.push("ID必须是数字");
					if (task.title && typeof task.title !== "string")
						taskErrors.push("标题必须是字符串");
			...
			expect(
				validationResults.invalid.some((r) => r.errors.includes("缺少ID")),
			).toBe(true);
		});
			...
			const generateValidationReport = (validationResults) => {
				const report = {
					summary: {
						...
						validityRate: Math.round(
							(validationResults.validCount / validationResults.total) * 100,
						),
					},
					details: {
						validTasks: validationResults.valid.map((r) => ({
							id: r.task.id,
							title: r.task.title,
						})),
				...
				const missingIds = validationResults.invalid.filter((r) =>
					r.errors.includes("缺少ID"),
				).length;
				if (missingIds > 0) {
	...
	describe("验证错误处理", () => {
		it("应该处理验证过程中的异常", () => {
			...
			expect(
				safeValidateTask({ id: 1, title: "有效任务", status: "pending" })
					.isValid,
			).toBe(true);
		});
		...
		it("应该验证验证规则的配置", () => {
			...
			expect(validateValidationRules(invalidRules).errors).toContain(
				"缺少必需的验证规则: dataTypes",
			);
			expect(validateValidationRules(invalidRules).errors).toContain(
				"缺少必需的验证规则: businessRules",
			);
		});
		...
		it("应该提供验证错误的详细诊断信息", () => {
			...
			const error1 = createDetailedError(
				task,
				"status",
				"invalid_value",
				"unknown",
			);
			const error2 = createDetailedError(
				task,
				"description",
				"missing",
				undefined,
			...
			expect(error2.suggestion).toContain(
				'为任务 "测试任务" 提供有效的 description',
			);
		});
	});
	...
	describe("验证配置管理", () => {
			...
			const createCustomValidator = (config) => {
				return (task) => {
					...
					if (
						...
						task.title &&
						task.title.length > config.maxTitleLength
					) {
					...
					if (
						...
						task.priority &&
						!config.allowedPriorities.includes(task.priority)
					) {
		...
		it("应该支持验证规则的启用/禁用", () => {
			...
			const minimalValidator = createConditionalValidator([
				"checkId",
				"checkTitle",
			]);
			const fullValidator = createConditionalValidator([
				"checkId",
				"checkTitle",
				"checkStatus",
				"checkDependencies",
