...
describe('manage-gitignore.js Unit Tests', () => {
	...
	describe('manageGitignoreFile function - File Operations with Spies', () => {
		...
		beforeEach(() => {
			...
			existsSyncSpy = jest.spyOn(fs, 'existsSync').mockImplementation(() => false)
			mockLog = jest.fn()
		})
...
`
				...
				expect(mockLog).toHaveBeenCalledWith('success', 'Created .gitignore with full template')
			})
...
more.txt`
				...
				expect(writtenContent).toMatch(/# Task files\s*[\r\n]+tasks\.json\s*[\r\n]+tasks\/ /)
			})
...
# tasks/`
				...
				const depsCount = (writtenContent.match(/# Dependencies/g) || []).length
