...
describe("配置管理功能验证", () => {
	...
	describe("配置文件数据结构", () => {
		...
		it("应该验证配置值的有效性", () => {
			const validateConfig = (config) => {
				...
				if (
					config.ai?.maxTokens &&
					(config.ai.maxTokens < 100 || config.ai.maxTokens > 32000)
				) {
				...
				if (
					config.task?.defaultPriority &&
					!validPriorities.includes(config.task.defaultPriority)
				) {
					errors.push("默认优先级无效");
				}
	...
	describe("配置项的更新操作", () => {
		it("应该能够更新单个配置项", () => {
			...
			const updatedConfig = updateConfigValue(
				config,
				"project.name",
				"updated-project",
			);
			...
			const updatedConfig3 = updateConfigValue(
				config,
				"models.fallback",
				"gpt-3.5-turbo",
			);
	...
	describe("配置安全性", () => {
		...
		it("应该验证配置文件的权限", () => {
			...
			const checkConfigFilePermissions = (configPath) => {
				try {
					...
					const permissions = (stats.mode & Number.parseInt("777", 8)).toString(
						8,
					);

					return {
						exists: true,
						isReadable: !!(stats.mode & Number.parseInt("400", 8)),
						isWritable: !!(stats.mode & Number.parseInt("200", 8)),
						isOwner,
						isGroup,
						permissions,
			...
			fs.statSync = jest.fn().mockReturnValue({
				uid: process.getuid(),
				gid: process.getgid(),
				mode: Number.parseInt("600", 8), // 只允许所有者读写
				isFile: () => true,
				isDirectory: () => false,
			});
		...
		it("应该支持配置加密存储", () => {
			...
			expect(encrypted.timestamp).toMatch(
	...
	describe("配置备份和恢复", () => {
		...
		it("应该能够从备份恢复配置", () => {
			...
			const restoredConfig = restoreConfigFromBackup(
			...
			expect(restoredConfig.originalBackupDate).toBe(
		...
		it("应该验证备份文件的完整性", () => {
			...
			expect(validateBackupIntegrity(invalidBackup).errors).toContain(
				"缺少backupDate字段",
			);
			expect(validateBackupIntegrity(invalidBackup).errors).toContain(
				"配置缺少项目名称",
