...
describe("任务删除功能验证", () => {
	...
	describe("基础删除功能", () => {
		it("应该能够删除单个任务", () => {
			...
			expect(
				remainingTasks.find((task) => task.id === taskIdToDelete),
			).toBeUndefined();
		...
		it("应该能够删除单个子任务", () => {
			...
			const updatedParentTask = {
				...
				subtasks: parentTask.subtasks.filter(
					(subtask) => subtask.id !== subtaskIdToDelete,
				),
			};

			expect(updatedParentTask.subtasks).toHaveLength(2);
			expect(updatedParentTask.subtasks.map((s) => s.id)).toEqual([
				"1.1",
				"1.3",
			]);
		});
	...
	describe("批量删除", () => {
		...
		it("应该在批量删除时正确清理依赖关系", () => {
			...
			const updatedTasks = tasks.map((task) => ({
				...task,
				dependencies: task.dependencies.filter(
					(depId) => !idsToDelete.includes(depId),
				),
			}));
			...
			const getAllChildrenIds = (taskId, hierarchy = taskHierarchy) => {
				...
				children.forEach((childId) => {
					allChildren = allChildren.concat(
						getAllChildrenIds(childId, hierarchy),
					);
				});
		...
		it("应该更新所有相关的依赖关系引用", () => {
			...
			const updatedTasks = tasks.map((task) => ({
				...
				dependencies: task.dependencies.map(
					(depId) => (depId === 4 ? 3 : depId), // ID 4变成ID 3
				),
			}));
		...
		it("应该处理权限检查失败", () => {
			...
			const canDeleteTask = (task, permissions = userPermissions) => {
				...
				if (
					...
					!permissions.canDeleteInProgressTasks
				) {
