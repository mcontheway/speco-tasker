...
describe('MCP集成功能验证', () => {
	...
	it('应该验证工具输入参数', () => {
				...
				Object.entries(schema.properties).forEach(([field, fieldSchema]) => {
					if (params[field] !== undefined) {
						...
						if (fieldSchema.enum && !fieldSchema.enum.includes(value)) {
							errors.push(`参数 ${field} 必须是以下值之一: ${fieldSchema.enum.join(', ')}`)
						}

						// 检查字符串长度
						if (expectedType === 'string' && fieldSchema.maxLength && value.length > fieldSchema.maxLength) {
							errors.push(`参数 ${field} 长度不能超过 ${fieldSchema.maxLength} 个字符`)
						}
					}
				})
			}
		...
		expect(validateToolInput(toolDefinition, missingRequired).isValid).toBe(false)
	...
	it('应该能够处理MCP通信错误', () => {
		const errorHandler = {
			handleConnectionError: (error, context = {}) => {
				...
				const retryable = ['ECONNREFUSED', 'ETIMEDOUT', 'ECONNRESET', 'ENOTFOUND'].includes(error.code)
		...
		const connectionError = errorHandler.handleConnectionError(connError, { host: 'localhost', port: 3000 })
