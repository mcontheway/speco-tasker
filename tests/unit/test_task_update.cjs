...
describe('任务更新功能验证', () => {
	...
	describe('任务状态管理', () => {
		it('应该能够更新任务状态为进行中', () => {
			...
			expect(['pending', 'in-progress', 'done', 'blocked', 'cancelled']).toContain(updatedTask.status)
		})
	...
	describe('批量更新', () => {
		...
		it('应该支持条件批量更新', () => {
			...
			const lowPriorityTasks = updatedTasks.filter(task => task.priority === 'low')
	...
	describe('更新历史追踪', () => {
		...
		it('应该支持撤销最近的更新', () => {
			...
			// 撤销到原始状态
			const revertedTask = { ...updatedTask, title: '原标题', status: 'pending' }
