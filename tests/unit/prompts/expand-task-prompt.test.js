...
describe('expand-task prompt template', () => {
	...
	test('default variant includes task context', () => {
		const { userPrompt } = promptManager.loadPrompt('expand-task', baseParams, 'default')
	...
	test('research variant includes task context', () => {
		...
		const { userPrompt } = promptManager.loadPrompt('expand-task', params, 'research')
	...
	test('complexity-report variant includes task context', () => {
		...
		const { userPrompt } = promptManager.loadPrompt('expand-task', params, 'complexity-report')
	...
	test('all variants request JSON format with subtasks array', () => {
		...
		variants.forEach((variant) => {
			const params =
				variant === 'complexity-report' ? { ...baseParams, expansionPrompt: 'test' } : baseParams

			const { systemPrompt, userPrompt } = promptManager.loadPrompt('expand-task', params, variant)
	...
	test('complexity-report variant fails without task context regression test', () => {
		...
		const titleOccurrences = (userPrompt.match(new RegExp(testTask.title, 'g')) || []).length
		const descriptionOccurrences = (userPrompt.match(new RegExp(testTask.description, 'g')) || [])
			.length
		...
		expect(userPrompt.length).toBeGreaterThan(params.expansionPrompt.length + 100)
	})
})
