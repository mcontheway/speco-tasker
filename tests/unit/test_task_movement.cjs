...
describe("任务移动功能验证", () => {
	...
	describe("层级结构调整", () => {
		it("应该能够将任务转换为子任务", () => {
			...
			const convertToSubtask = (tasks, taskId, parentId) => {
				...
				// 添加到父任务的子任务列表
				newTasks[
					parentIndex > taskIndex ? parentIndex - 1 : parentIndex
	...
	describe("移动验证和边界条件", () => {
		...
		it("应该防止无效的层级转换", () => {
			...
			const validateHierarchyChange = (tasks, taskId, newParentId) => {
				...
				if (task && task.subtasks.some((st) => st.id === newParentId))
					return false;
	...
	describe("批量移动操作", () => {
		...
		it("应该支持条件批量移动", () => {
			...
			// 将所有高优先级任务移动到列表开头
			const result = conditionalBatchMove(
				tasks,
	...
	describe("移动历史和撤销", () => {
		...
		it("应该支持撤销移动操作", () => {
			...
			// 执行移动
			const result = moveTaskWithHistory(tasks, 2, 0);
			expect(result[0].id).toBe(2);
			...
			const moveNestedTask = (structure, fromPath, toPath) => {
				const findTaskByPath = (obj, path) => {
					...
					if (obj.id === current && obj.subtasks) {
						return findTaskByPath(
							obj.subtasks.find((st) => st.id === path[1]),
							rest,
						);
					}
					return null;
