...
describe("任务依赖功能验证", () => {
	...
	describe("任务依赖数据结构", () => {
		...
		it("应该验证依赖关系的数据类型和约束", () => {
			...
			expect(validTask.dependents.every((dep) => typeof dep === "number")).toBe(
				true,
			);
			expect(
	...
	describe("依赖关系维护", () => {
		...
		it("应该在移动任务时更新依赖关系", () => {
			...
			const updatedTasks = tasks.map((task) => ({
				...
				dependents: task.dependents.map((depId) => (depId === 2 ? 5 : depId)),
			}));
	...
	describe("依赖关系边界情况处理", () => {
		...
		it("应该处理包含null或undefined值的依赖关系", () => {
			...
			const cleanDependencies = taskWithInvalidDeps.dependencies.filter(
				(dep) => dep != null && typeof dep === "number",
			);

			const cleanDependents = taskWithInvalidDeps.dependents.filter(
				(dep) => dep != null && typeof dep === "number",
			);
