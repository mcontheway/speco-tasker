...
describe('VS Code Integration', () => {
	...
	function mockCreateVSCodeStructure() {
		...
		fs.writeFileSync(path.join(tempDir, '.vscode', 'mcp.json'), JSON.stringify(mcpConfig, null, 2))

		// Create sample instruction files
		const instructionFiles = ['vscode_rules.md', 'dev_workflow.md', 'self_improve.md']
...
This is a VS Code custom instruction file.`
			...
			fs.writeFileSync(path.join(tempDir, '.github', 'instructions', file), content)
		}
...
Task Master specific VS Code instruction.`
			...
			fs.writeFileSync(path.join(tempDir, '.github', 'instructions', 'taskmaster', file), content)
		}
	}
	...
	test('creates VS Code instruction files with applyTo patterns', () => {
		...
		const mainInstructionFiles = ['vscode_rules.md', 'dev_workflow.md', 'self_improve.md']
	...
	test('creates taskmaster specific instruction files', () => {
		...
		for (const file of taskmasterFiles) {
			const expectedPath = path.join(tempDir, '.github', 'instructions', 'taskmaster', file)
			expect(fs.writeFileSync).toHaveBeenCalledWith(
				expectedPath,
				expect.stringContaining('applyTo:')
			)
	...
	test('MCP configuration includes correct structure for VS Code', () => {
		...
		expect(mcpConfig.mcpServers['task-master-ai']).toHaveProperty('command', 'node')
		expect(mcpConfig.mcpServers['task-master-ai']).toHaveProperty('args')
		expect(mcpConfig.mcpServers['task-master-ai'].args).toContain('mcp-server/src/index.js')
	})
	...
	describe('Schema Integration', () => {
		...
		test('schema integration handles errors gracefully', async () => {
			// Arrange
			mockSetupSchemaIntegration.mockRejectedValue(new Error('Schema setup failed'))

			// Act & Assert - Should propagate the error
			await expect(vscodeProfile.onAddRulesProfile(tempDir)).rejects.toThrow('Schema setup failed')
		})
	})
})
