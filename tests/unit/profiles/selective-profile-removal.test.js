...
import { getRulesProfile, removeProfileRules } from '../../../src/utils/rule-transformer.js'
...
describe('Selective Rules Removal', () => {
	...
	describe('removeProfileRules - Selective File Removal', () => {
		it('should only remove Task Master files, preserving existing rules', () => {
			...
			expect(mockRmSync).not.toHaveBeenCalledWith(path.join(projectRoot, '.cursor/rules'), {
				recursive: true,
				force: true
			})

			// Verify profile directory was NOT removed
			expect(mockRmSync).not.toHaveBeenCalledWith(path.join(projectRoot, '.cursor'), {
				recursive: true,
				force: true
			})
		})
		...
		it('should remove empty rules directory if only Task Master files existed', () => {
			...
			if (result.success) {
				...
				expect(mockRmSync).toHaveBeenCalledWith(path.join(projectRoot, '.cursor/rules'), {
					recursive: true,
					force: true
				})
			} else {
		...
		it('should remove entire profile directory if completely empty and all rules were Task Master rules and MCP config deleted', () => {
			...
			expect(mockRmSync).toHaveBeenCalledWith(path.join(projectRoot, '.cursor'), {
				recursive: true,
				force: true
			})
		})
	...
	describe('removeTaskMasterMCPConfiguration - Selective MCP Removal', () => {
		it('should only remove Task Master from MCP config, preserving other servers', () => {
			...
			const result = removeTaskMasterMCPConfiguration(projectRoot, mcpConfigPath)
		...
		it('should delete entire MCP config if Task Master is the only server', () => {
			...
			expect(mockRmSync).toHaveBeenCalledWith(path.join(projectRoot, mcpConfigPath), {
				force: true
			})
	...
	describe('Integration - Full Profile Removal with Preservation', () => {
		it('should handle complete removal scenario with notices', () => {
			...
			mockExistsSync.mockImplementation((filePath) => {
				...
				if (filePath === path.join(projectRoot, '.cursor/rules/taskmaster/dev_workflow.mdc'))
					return false
				if (filePath === path.join(projectRoot, '.cursor/rules/self_improve.mdc')) return false
				if (filePath === path.join(projectRoot, '.cursor/rules/taskmaster/taskmaster.mdc'))
					return false
				return false
			})
			...
			expect(result.notice).toContain('preserved other MCP server configurations')
			expect(result.mcpResult.hasOtherServers).toBe(true)
			expect(result.profileDirRemoved).toBe(false)
		})
	})
