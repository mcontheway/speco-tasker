...
describe('Gemini Profile Integration', () => {
	...
	function mockCreateGeminiStructure() {
		...
		fs.writeFileSync(path.join(tempDir, '.gemini', 'settings.json'), settingsContent)
	}
	...
	test('uses settings.json instead of mcp.json', () => {
		...
		const writeFileCalls = fs.writeFileSync.mock.calls
		const mcpJsonCalls = writeFileCalls.filter((call) => call[0].toString().includes('mcp.json'))
	...
	test('renames AGENTS.md to GEMINI.md', () => {
		...
		const writeFileCalls = fs.writeFileSync.mock.calls
		const geminiMdCall = writeFileCalls.find((call) => call[0].toString().includes('GEMINI.md'))
