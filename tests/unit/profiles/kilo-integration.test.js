...
describe('Kilo Integration', () => {
	...
	function mockCreateKiloStructure() {
		...
		const kiloModes = ['architect', 'ask', 'orchestrator', 'code', 'debug', 'test']
	...
	test('creates all required .kilo directories', () => {
		...
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.kilo', 'rules'), {
			recursive: true
		})

		// Verify all mode directories are created
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.kilo', 'rules-architect'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.kilo', 'rules-ask'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.kilo', 'rules-orchestrator'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.kilo', 'rules-code'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.kilo', 'rules-debug'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.kilo', 'rules-test'), {
			recursive: true
		})
	})
	...
	test('creates additional required Kilo directories', () => {
		...
		// Assert
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.kilo', 'config'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.kilo', 'templates'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.kilo', 'logs'), {
			recursive: true
		})
	})
})
