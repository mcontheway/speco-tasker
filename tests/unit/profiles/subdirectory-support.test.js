...
describe('Rules Subdirectory Support Feature', () => {
	it('should support taskmaster subdirectories only for Cursor profile', () => {
		...
		expect(cursorProfile.fileMap['rules/dev_workflow.mdc']).toBe('taskmaster/dev_workflow.mdc')
		expect(cursorProfile.fileMap['rules/taskmaster.mdc']).toBe('taskmaster/taskmaster.mdc')
	})
	...
	it('should not use taskmaster subdirectories for other profiles', () => {
		...
		profiles.forEach((profileName) => {
			...
			expect(profile.fileMap['rules/dev_workflow.mdc']).toBe(`dev_workflow${expectedExt}`)
			expect(profile.fileMap['rules/taskmaster.mdc']).toBe(`taskmaster${expectedExt}`)
		})
	})

	it('should have supportsRulesSubdirectories property accessible on all profiles', () => {
		const allProfiles = ['cursor', 'roo', 'vscode', 'cline', 'windsurf', 'trae']
