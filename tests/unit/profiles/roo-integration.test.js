...
describe('Roo Integration', () => {
	...
	test('creates all required .roo directories', () => {
		...
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.roo', 'rules'), {
			recursive: true
		})

		// Verify all mode directories are created
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.roo', 'rules-architect'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.roo', 'rules-ask'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.roo', 'rules-orchestrator'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.roo', 'rules-code'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.roo', 'rules-debug'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.roo', 'rules-test'), {
			recursive: true
		})
	})
	...
	test('creates additional required Roo directories', () => {
		...
		// Assert
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.roo', 'config'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.roo', 'templates'), {
			recursive: true
		})
		expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, '.roo', 'logs'), {
			recursive: true
		})
	})
})
