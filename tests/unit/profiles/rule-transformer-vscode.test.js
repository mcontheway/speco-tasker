...
describe.skip('Rule Transformer - VSCode Profile', () => {
...
Also has references to .mdc files and cursor rules.`
		...
		const result = convertRuleToProfileRule('source.mdc', 'target.md', vscodeProfile)
...
Files are in the .cursor/rules directory and we should reference the rules directory.`
		...
		// Call the actual function
		const result = convertRuleToProfileRule('source.mdc', 'target.instructions.md', vscodeProfile)
		...
		expect(transformedContent).toContain('applyTo: ".github/instructions/*.md"') // globs -> applyTo with path transformation
		expect(transformedContent).toContain('(.github/instructions/dev_workflow.instructions.md)') // File path transformation - no taskmaster subdirectory for VS Code
		expect(transformedContent).toContain('(.github/instructions/taskmaster.instructions.md)') // File path transformation - no taskmaster subdirectory for VS Code
...
---`
		...
		expect(transformedContent).toContain('applyTo: ".github/instructions/*.md"') // Path transformation applied
...
Update your .cursor/mcp.json file accordingly.`
	...
	it('should handle file read errors', () => {
		...
		// Call the actual function
		const result = convertRuleToProfileRule('nonexistent.mdc', 'target.md', vscodeProfile)
		...
		expect(mockConsoleError).toHaveBeenCalledWith('Error converting rule file: File not found')
	})
	...
	it('should handle file write errors', () => {
		...
		// Verify error was logged
		expect(mockConsoleError).toHaveBeenCalledWith('Error converting rule file: Permission denied')
	})
	...
	it('should create target directory if it does not exist', () => {
		...
		expect(mockMkdirSync).toHaveBeenCalledWith('.github/instructions/deep/path', {
			recursive: true
		})
	})
})
