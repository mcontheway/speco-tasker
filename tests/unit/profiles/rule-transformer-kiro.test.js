...
Also has references to .mdc files.`
		...
		// Call the function
		const result = convertRuleToProfileRule('test-source.mdc', 'test-target.md', kiroProfile)
...
Use the .mdc extension for all rule files.`
	...
	describe('onPostConvert lifecycle hook', () => {
		...
		it('should copy hook files when kiro-hooks directory exists', () => {
			...
			const hookFiles = ['tm-test-hook1.kiro.hook', 'tm-test-hook2.kiro.hook', 'not-a-hook.txt']
