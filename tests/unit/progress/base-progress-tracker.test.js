...
jest.unstable_mockModule('../../../src/progress/cli-progress-factory.js', () => ({
	newMultiBar: jest.fn(() => ({
		create: jest.fn(() => ({
			update: jest.fn()
		})),
...
const { newMultiBar } = await import('../../../src/progress/cli-progress-factory.js')
const { BaseProgressTracker } = await import('../../../src/progress/base-progress-tracker.js')
...
describe('BaseProgressTracker', () => {
	...
	beforeEach(() => {
		...
		mockMultiBar = {
			create: jest.fn().mockReturnValueOnce(mockTimeTokensBar).mockReturnValueOnce(mockProgressBar),
			stop: jest.fn()
		}
