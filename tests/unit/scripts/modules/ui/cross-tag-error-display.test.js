...
describe('Cross-Tag Error Display Functions', () => {
	...
	describe('displayCrossTagDependencyError', () => {
		it('should display cross-tag dependency error with conflicts', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move tasks from "in-progress" to "done"')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Resolution options:'))
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('--with-dependencies'))
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('--ignore-dependencies'))
		})
	...
	describe('displaySubtaskMoveError', () => {
		it('should display subtask movement restriction error', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move subtask 5.2 directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('Subtask movement restriction:')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('• Subtasks cannot be moved directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Resolution options:'))
			expect(mockConsoleLog).toHaveBeenCalledWith(
		...
		it('should handle nested subtask IDs (three levels)', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move subtask 5.2.1 directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
		...
		it('should handle deeply nested subtask IDs (four levels)', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move subtask 10.3.2.1 directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
		...
		it('should handle invalid subtask ID format gracefully', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move subtask invalid-id directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
		...
		it('should handle empty subtask ID', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining(`remove-subtask --id=${formatTaskIdForDisplay('')} --convert`)
			)
		})
		...
		it('should handle null subtask ID', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move subtask null directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
		...
		it('should handle undefined subtask ID', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move subtask undefined directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
		...
		it('should handle special characters in subtask ID', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move subtask 5.2@test directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
		...
		it('should handle numeric subtask IDs', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move subtask 123.456 directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
		...
		it('should handle identical source and target tags', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move subtask 7.3 directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
		...
		it('should handle empty tag names', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move subtask 9.1 directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('• Source tag: ""'))
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('• Target tag: ""'))
		})
		...
		it('should handle null tag names', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move subtask 12.4 directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('• Source tag: "null"'))
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('• Target tag: "null"'))
		})

		it('should handle complex tag names with special characters', () => {
			displaySubtaskMoveError('3.2.1', 'feature/user-auth@v2.0', 'production@stable')

			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('❌ Cannot move subtask 3.2.1 directly between tags')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
		...
		it('should handle very long subtask IDs', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining(`❌ Cannot move subtask ${longId} directly between tags`)
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
	...
	describe('displayInvalidTagCombinationError', () => {
		it('should display invalid tag combination error', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Error details:'))
			expect(mockConsoleLog).toHaveBeenCalledWith(
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('• Reason: Source and target tags are identical')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Resolution options:'))
		})
	})
	...
	describe('displayDependencyValidationHints', () => {
		it('should display general hints by default', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Helpful hints:'))
			expect(mockConsoleLog).toHaveBeenCalledWith(
		...
		it('should display before-move hints', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Helpful hints:'))
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('💡 Tip: Run "task-master validate-dependencies"')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
		...
		it('should display after-error hints', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Helpful hints:'))
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('🔧 Quick fix: Run "task-master validate-dependencies"')
			)
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('🔧 Quick fix: Use "task-master fix-dependencies"')
			)
		})
