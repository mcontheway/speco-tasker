...
describe('Circular Dependency Scenarios', () => {
	describe('Circular Cross-Tag Dependencies', () => {
		...
		it('should detect circular dependencies across tags', () => {
			...
			const conflicts = findCrossTagDependencies([allTasks[0]], 'backlog', 'in-progress', allTasks)
		...
		it('should return canMove: false for circular dependencies', () => {
			const result = canMoveWithDependencies('1', 'backlog', 'in-progress', allTasks)
	...
	describe('Complex Dependency Chains', () => {
		...
		it('should find all dependencies in complex chain', () => {
			...
			expect(conflicts.some((c) => c.taskId === 1 && c.dependencyId === 2)).toBe(true)
			expect(conflicts.some((c) => c.taskId === 1 && c.dependencyId === 3)).toBe(true)
		})

		it('should get all dependent task IDs in complex chain', () => {
			const conflicts = findCrossTagDependencies([allTasks[0]], 'backlog', 'in-progress', allTasks)
			const dependentIds = getDependentTaskIds([allTasks[0]], conflicts, allTasks)
	...
	describe('Mixed Dependency Types', () => {
		...
		it('should handle mixed task and subtask dependencies', () => {
			...
			expect(conflicts).toHaveLength(2)
			expect(conflicts.some((c) => c.taskId === 1 && c.dependencyId === 2)).toBe(true)
			expect(conflicts.some((c) => c.taskId === 1 && c.dependencyId === '3.1')).toBe(true)
		})
	})
	...
	describe('Edge Cases and Error Conditions', () => {
		...
		it('should handle invalid tag names', () => {
			expect(() => {
				findCrossTagDependencies([allTasks[0]], 'invalid-tag', 'in-progress', allTasks)
			}).not.toThrow()
		})
		...
		it('should handle null/undefined dependencies', () => {
			...
			expect(() => {
				findCrossTagDependencies([taskWithNullDeps], 'backlog', 'in-progress', allTasks)
			}).not.toThrow()
		})
