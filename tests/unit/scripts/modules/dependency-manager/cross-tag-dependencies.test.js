...
describe('Cross-Tag Dependency Validation', () => {
	describe('validateCrossTagMove', () => {
		...
		it('should allow move when no dependencies exist', () => {
			...
			const result = validateCrossTagMove(task, 'backlog', 'in-progress', mockAllTasks)
		...
		it('should allow move when dependencies are in target tag', () => {
			...
			const updatedTasks2 = updatedTasks.map((t) => (t.id === 1 ? { ...t, tag: 'done' } : t))
			const result = validateCrossTagMove(task, 'in-progress', 'done', updatedTasks2)
		...
		it('should handle multiple dependencies correctly', () => {
			...
			const result = validateCrossTagMove(task, 'backlog', 'done', mockAllTasks)
		...
		it('should throw error for invalid task parameter', () => {
			expect(() => validateCrossTagMove(null, 'backlog', 'in-progress', mockAllTasks)).toThrow(
				'Task parameter must be a valid object'
			)
		})

		it('should throw error for invalid source tag', () => {
			const task = { id: 1, dependencies: [], title: 'Task 1' }
			expect(() => validateCrossTagMove(task, '', 'in-progress', mockAllTasks)).toThrow(
		...
		it('should throw error for invalid target tag', () => {
			const task = { id: 1, dependencies: [], title: 'Task 1' }
			expect(() => validateCrossTagMove(task, 'backlog', null, mockAllTasks)).toThrow(
		...
		it('should throw error for invalid allTasks parameter', () => {
			const task = { id: 1, dependencies: [], title: 'Task 1' }
			expect(() => validateCrossTagMove(task, 'backlog', 'in-progress', 'not-an-array')).toThrow(
	...
	describe('findCrossTagDependencies', () => {
		...
		it('should find cross-tag dependencies for multiple tasks', () => {
			...
			const conflicts = findCrossTagDependencies(sourceTasks, 'backlog', 'done', mockAllTasks)
		...
		it('should throw error for invalid sourceTasks parameter', () => {
			expect(() =>
				findCrossTagDependencies('not-an-array', 'backlog', 'done', mockAllTasks)
			).toThrow('Source tasks parameter must be an array')
		})

		it('should throw error for invalid source tag', () => {
			const sourceTasks = [{ id: 1, dependencies: [], title: 'Task 1' }]
			expect(() => findCrossTagDependencies(sourceTasks, '', 'done', mockAllTasks)).toThrow(
				'Source tag must be a valid string'
			)
		})
		...
		it('should throw error for invalid target tag', () => {
			const sourceTasks = [{ id: 1, dependencies: [], title: 'Task 1' }]
			expect(() => findCrossTagDependencies(sourceTasks, 'backlog', null, mockAllTasks)).toThrow(
				'Target tag must be a valid string'
			)
		})
	...
	describe('getDependentTaskIds', () => {
		...
		it('should return dependent task IDs', () => {
			...
			const crossTagDependencies = [{ taskId: 1, dependencyId: 2, dependencyTag: 'backlog' }]
			const dependentIds = getDependentTaskIds(sourceTasks, crossTagDependencies, mockAllTasks)
		...
		it('should throw error for invalid sourceTasks parameter', () => {
			const crossTagDependencies = []
			expect(() => getDependentTaskIds('not-an-array', crossTagDependencies, mockAllTasks)).toThrow(
				'Source tasks parameter must be an array'
			)
		})

		it('should throw error for invalid crossTagDependencies parameter', () => {
			const sourceTasks = [{ id: 1, dependencies: [], title: 'Task 1' }]
			expect(() => getDependentTaskIds(sourceTasks, 'not-an-array', mockAllTasks)).toThrow(
				'Cross tag dependencies parameter must be an array'
			)
		})
		...
		it('should throw error for invalid allTasks parameter', () => {
			const sourceTasks = [{ id: 1, dependencies: [], title: 'Task 1' }]
			const crossTagDependencies = []
			expect(() => getDependentTaskIds(sourceTasks, crossTagDependencies, 'not-an-array')).toThrow(
				'All tasks parameter must be an array'
			)
		})
	})
	...
	describe('validateSubtaskMove', () => {
		it('should throw error for subtask movement', () => {
			expect(() => validateSubtaskMove('1.2', 'backlog', 'in-progress')).toThrow(
				'Cannot move subtask 1.2 directly between tags'
			)
		})

		it('should allow regular task movement', () => {
			expect(() => validateSubtaskMove('1', 'backlog', 'in-progress')).not.toThrow()
		})
	...
	describe('canMoveWithDependencies', () => {
		...
		it('should return canMove: true when no conflicts exist', () => {
			const result = canMoveWithDependencies('2', 'backlog', 'in-progress', mockAllTasks)
		...
		it('should return canMove: false when conflicts exist', () => {
			const result = canMoveWithDependencies('1', 'backlog', 'in-progress', mockAllTasks)
		...
		it('should return canMove: false when task not found', () => {
			const result = canMoveWithDependencies('999', 'backlog', 'in-progress', mockAllTasks)
		...
		it('should throw error for invalid taskId parameter', () => {
			expect(() => canMoveWithDependencies(null, 'backlog', 'in-progress', mockAllTasks)).toThrow(
				'Task ID must be a valid string'
			)
		})

		it('should throw error for invalid source tag', () => {
			expect(() => canMoveWithDependencies('1', '', 'in-progress', mockAllTasks)).toThrow(
				'Source tag must be a valid string'
			)
		})

		it('should throw error for invalid target tag', () => {
			expect(() => canMoveWithDependencies('1', 'backlog', null, mockAllTasks)).toThrow(
				'Target tag must be a valid string'
			)
		})

		it('should throw error for invalid allTasks parameter', () => {
			expect(() => canMoveWithDependencies('1', 'backlog', 'in-progress', 'not-an-array')).toThrow(
				'All tasks parameter must be an array'
			)
		})
	})
