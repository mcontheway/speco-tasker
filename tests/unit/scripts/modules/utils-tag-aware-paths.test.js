...
describe('getTagAwareFilePath utility function', () => {
	...
	it('should return base path for main tag', () => {
		const result = getTagAwareFilePath(basePath, 'main', projectRoot)
	...
	it('should handle different file extensions', () => {
		...
		const expected = path.join(projectRoot, '.taskmaster/reports/export_dev-branch.csv')
		expect(result).toBe(expected)
	})
	...
	it('should handle paths without extensions', () => {
		...
		const expected = path.join(projectRoot, '.taskmaster/reports/summary_test-tag')
		expect(result).toBe(expected)
	})
	...
	it('should use default project root when not provided', () => {
		...
		const expected = path.join('.', '.taskmaster/reports/task-complexity-report_feature-tag.json')
		expect(result).toBe(expected)
	})
