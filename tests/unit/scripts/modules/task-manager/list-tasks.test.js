...
const { readJSON, log, readComplexityReport, addComplexityToTask, stripAnsiCodes } = await import(
	'../../../../../scripts/modules/utils.js'
)
const { displayTaskList } = await import('../../../../../scripts/modules/ui.js')
...
describe('listTasks', () => {
	...
	describe('Comma-separated status filtering', () => {
		test('should filter tasks by multiple statuses separated by commas', async () => {
			...
			expect(result.tasks.map((t) => t.status)).toEqual(expect.arrayContaining(['done', 'pending']))
		})
		...
		test('should filter tasks by three or more statuses', async () => {
			...
			expect(statusValues).toEqual(expect.arrayContaining(['done', 'pending', 'in-progress']))
		...
		test('should filter by review status in comma-separated list', async () => {
			...
			expect(result.tasks).toHaveLength(2)
			const statusValues = result.tasks.map((task) => task.status)
			expect(statusValues).toEqual(expect.arrayContaining(['review', 'cancelled']))
	...
	describe('Compact output format', () => {
		test('should output compact format when outputFormat is compact', async () => {
			...
			expect(cleanOutput).toContain('2 pending Implement Core Features (high) → 1')
		...
		test('should format dependencies correctly with shared helper', async () => {
			...
			expect(cleanOutput).toContain('1 pending Task with no dependencies (medium)')
