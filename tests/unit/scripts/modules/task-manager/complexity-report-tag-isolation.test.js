...
jest.unstable_mockModule('../../../../../src/utils/path-utils.js', () => ({
	...
	findTasksPath: jest.fn(() => '/mock/project/root/.taskmaster/tasks/tasks.json'),
...
jest.unstable_mockModule('../../../../../scripts/modules/utils.js', () => ({
	...
	createTagAwareFilePath: jest.fn((basePath, tag, projectRoot) => {
		if (tag && tag !== 'master') {
			...
			return path.join(projectRoot || '.', dir, `${name}_${slugifiedTag}${ext}`)
		}
		return path.join(projectRoot || '.', basePath)
	}),
...
jest.unstable_mockModule('../../../../../scripts/modules/utils/contextGatherer.js', () => {
	...
	return {
		...
		createContextGatherer: jest.fn((projectRoot, tag) => new MockContextGatherer(projectRoot, tag))
	}
})
...
const { resolveComplexityReportOutputPath, findComplexityReportPath } = await import(
...
describe('Complexity Report Tag Isolation', () => {
	...
	beforeEach(() => {
		...
		resolveComplexityReportOutputPath.mockImplementation((explicitPath, args) => {
			const tag = args?.tag
			if (explicitPath) {
				return explicitPath
			}
	...
	describe('Path Resolution Tag Isolation', () => {
		test('should resolve master tag to default filename', () => {
			...
			expect(result).toBe(
				path.join(projectRoot, '.taskmaster/reports', 'task-complexity-report.json')
			)
		})
		...
		test('should resolve non-master tag to tag-specific filename', () => {
			...
			expect(result).toBe(
				path.join(projectRoot, '.taskmaster/reports', 'task-complexity-report_feature-auth.json')
			)
		})
