...
const { readJSON, writeJSON, log, findProjectRoot, ensureTagMetadata } = await import(
	'../../../../../scripts/modules/utils.js'
)
const { formatDependenciesWithStatus } = await import('../../../../../scripts/modules/ui.js')
...
describe('generateTaskFiles', () => {
	...
	test('should generate task files from tasks.json - working test', async () => {
		...
		expect(fs.writeFileSync).toHaveBeenCalledWith('tasks/task_001.txt', expect.any(String))
		expect(fs.writeFileSync).toHaveBeenCalledWith('tasks/task_002.txt', expect.any(String))
		expect(fs.writeFileSync).toHaveBeenCalledWith('tasks/task_003.txt', expect.any(String))
	})

	test('should format dependencies with status indicators', async () => {
		// Set up mocks
		fs.existsSync.mockReturnValue(true)
		formatDependenciesWithStatus.mockReturnValue('✅ Task 1 (done), ⏱️ Task 2 (pending)')
