...
const { readJSON, promptYesNo } = await import('../../../../../scripts/modules/utils.js')
...
const { createParsePrdTracker } = await import('../../../../../src/progress/parse-prd-tracker.js')
...
describe('parsePRD', () => {
				...
				mainResult: {
					get partialObjectStream() {
						return (async function* () {
							...
							yield {
								tasks: [sampleClaudeResponse.tasks[0], sampleClaudeResponse.tasks[1]]
							}
	...
	test('should parse a PRD file and generate tasks', async () => {
		...
		expect(fs.default.readFileSync).toHaveBeenCalledWith('path/to/prd.txt', 'utf8')
	...
	describe('Streaming vs Non-Streaming Modes', () => {
		...
		test.skip('should not re-throw non-streaming errors during fallback - needs rewrite for streamObject', async () => {
			...
			streamTextService.mockRejectedValueOnce(new Error('AI API rate limit exceeded'))
