...
const { validateCrossTagMove, findCrossTagDependencies, getDependentTaskIds, validateSubtaskMove } =
...
describe('Cross-Tag Task Movement', () => {
	...
	describe('validateCrossTagMove', () => {
		it('should allow move when no dependencies exist', () => {
			...
			const result = validateCrossTagMove(task, 'backlog', 'in-progress', allTasks)
	...
	describe('findCrossTagDependencies', () => {
		it('should find cross-tag dependencies for multiple tasks', () => {
			...
			const conflicts = findCrossTagDependencies(sourceTasks, 'backlog', 'in-progress', allTasks)

			expect(conflicts).toHaveLength(2)
			expect(conflicts.some((c) => c.taskId === 1 && c.dependencyId === 2)).toBe(true)
			expect(conflicts.some((c) => c.taskId === 3 && c.dependencyId === 1)).toBe(true)
		})
	})
	...
	describe('getDependentTaskIds', () => {
		it('should return dependent task IDs', () => {
			...
			const dependentTaskIds = getDependentTaskIds(sourceTasks, crossTagDependencies, allTasks)
	...
	describe('moveTasksBetweenTags', () => {
		...
		it('should throw error for cross-tag dependencies by default', async () => {
			...
			await expect(
				moveTasksBetweenTags(mockTasksPath, [1], 'backlog', 'in-progress', {}, mockContext)
			).rejects.toThrow('Cannot move tasks: 1 cross-tag dependency conflicts found')
		...
		it('should throw error for subtask movement', async () => {
			...
			await expect(
				moveTasksBetweenTags(mockTasksPath, ['1.2'], 'backlog', 'in-progress', {}, mockContext)
		...
		it('should throw error for invalid source tag', async () => {
			...
			await expect(
				moveTasksBetweenTags(mockTasksPath, [1], 'non-existent-tag', 'in-progress', {}, mockContext)
