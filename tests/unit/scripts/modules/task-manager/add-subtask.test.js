...
jest.unstable_mockModule('../../../../../scripts/modules/task-manager.js', () => mockTaskManager)
...
const addSubtask = (await import('../../../../../scripts/modules/task-manager/add-subtask.js'))
	.default
...
describe('addSubtask function', () => {
	...
	test('should convert an existing task to a subtask', async () => {
		...
		const context = {}
		const convertedSubtask = await addSubtask('tasks.json', '1', '2', null, true, context)
	...
	test('should throw an error if parent task does not exist', async () => {
		...
		const context = {}
		await expect(
			addSubtask('tasks.json', '99', null, { title: 'New Subtask' }, true, context)
	...
	test('should throw an error if trying to convert a non-existent task', async () => {
		...
		const context = {}
		await expect(addSubtask('tasks.json', '1', '99', null, true, context)).rejects.toThrow(
	...
	test('should throw an error for circular dependency', async () => {
		...
		mockTaskManager.isTaskDependentOn.mockImplementation((tasks, parentTask, existingTaskIdNum) => {
			return parentTask.id === 1 && existingTaskIdNum === 2
		})
		const context = {}
		await expect(addSubtask('tasks.json', '1', '2', null, true, context)).rejects.toThrow(
