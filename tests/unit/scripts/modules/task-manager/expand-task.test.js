		...
		mainResult: JSON.stringify({
			subtasks: [
				...
				},
				{
					...
					dependencies: [1],
					details: 'Create main classes, implement business logic, set up data models',
					status: 'pending',
				...
				},
				{
					...
					dependencies: [2],
					details: 'Design UI components, implement responsive layouts, add user interactions',
					status: 'pending',
...
jest.unstable_mockModule('../../../../../scripts/modules/utils/fuzzyTaskSearch.js', () => ({
...
const { getDefaultSubtasks } = await import('../../../../../scripts/modules/config-manager.js')
...
describe('expandTask', () => {
	...
	describe('Basic Functionality', () => {
		test('should expand a task with AI-generated subtasks', async () => {
			...
			// Act
			const result = await expandTask(tasksPath, taskId, numSubtasks, false, '', context, false)

			// Assert
			expect(readJSON).toHaveBeenCalledWith(tasksPath, '/mock/project/root', undefined)
	...
	describe('Tag Handling (The Critical Bug Fix)', () => {
		test('should preserve tagged structure when expanding with default tag', async () => {
			...
			expect(readJSON).toHaveBeenCalledWith(tasksPath, '/mock/project/root', 'master')
			expect(writeJSON).toHaveBeenCalledWith(
				tasksPath,
				expect.objectContaining({
					tag: 'master',
		...
		test('should preserve tagged structure when expanding with non-default tag', async () => {
			...
			expect(readJSON).toHaveBeenCalledWith(tasksPath, '/mock/project/root', 'feature-branch')
			expect(writeJSON).toHaveBeenCalledWith(
				tasksPath,
				expect.objectContaining({
					tag: 'feature-branch',
	...
	describe('Complexity Report Integration (Tag-Specific)', () => {
		test('should use tag-specific complexity report when available', async () => {
			// Arrange
			const { getPromptManager } = await import('../../../../../scripts/modules/prompt-manager.js')
			const mockLoadPrompt = jest.fn().mockResolvedValue({
				systemPrompt: 'Generate exactly 5 subtasks for complexity report',
				userPrompt: 'Please break this task into 5 parts\n\nUser provided context'
			})
			...
			const context = {
				...
				projectRoot: '/mock/project/root',
				tag: 'feature-branch',
				complexityReportPath: '/mock/project/root/task-complexity-report_feature-branch.json'
			}
	...
	describe('Error Handling', () => {
		test('should handle non-existent task ID', async () => {
			...
			// Act & Assert
			await expect(expandTask(tasksPath, taskId, 3, false, '', context, false)).rejects.toThrow(
				'Task 999 not found'
			)
		...
		test('should expand tasks regardless of status (including done tasks)', async () => {
			...
			// Act
			const result = await expandTask(tasksPath, taskId, 3, false, '', context, false)
	...
	describe('Edge Cases', () => {
		...
		test('should handle additional context correctly', async () => {
			...
			// Act
			await expandTask(tasksPath, taskId, 3, false, additionalContext, context, false)

			// Assert - Should pass separate context parameters to prompt manager
			expect(mockLoadPrompt).toHaveBeenCalledWith(
				'expand-task',
				expect.objectContaining({
					additionalContext: expect.stringContaining('Use React hooks and TypeScript'),
					gatheredContext: expect.stringContaining('Mock project context from files')
				}),
				expect.any(String)
			)
			...
			expect(parameters.additionalContext).toContain('Use React hooks and TypeScript')
			expect(parameters.gatheredContext).toContain('Mock project context from files')
		})
	...
	describe('Dynamic Subtask Generation', () => {
		...
		test('should accept 0 as valid numSubtasks value for dynamic generation', async () => {
			...
			const result = await expandTask(tasksPath, taskId, 0, false, '', context, false)
		...
		test('should use dynamic prompting when numSubtasks is 0', async () => {
			...
			const mockLoadPrompt = jest.fn().mockResolvedValue({
				...
				userPrompt: 'Break down this task into an appropriate number of specific subtasks'
			})
			getPromptManager.mockReturnValue({
				loadPrompt: mockLoadPrompt
			})
			...
			expect(callArgs.systemPrompt).toContain('an appropriate number of specific subtasks')
		})
