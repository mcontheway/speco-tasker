...
jest.unstable_mockModule('../../../../../scripts/modules/ai-services-unified.js', () => ({
	generateTextService: jest.fn().mockResolvedValue({
		mainResult: 'Test research result with ```javascript\nconsole.log("test");\n```',
		telemetryData: {}
	})
}))
...
describe('performResearch project root validation', () => {
	it('throws error when project root cannot be determined', async () => {
		...
		await expect(performResearch('Test query', {}, {}, 'json', false)).rejects.toThrow(
...
describe('performResearch tag-aware functionality', () => {
	...
	beforeEach(() => {
		...
		mockFuzzySearch = {
			findRelevantTasks: jest.fn(() => [{ id: 1, title: 'Task 1', description: 'Description 1' }]),
	...
	describe('tag parameter passing to ContextGatherer', () => {
		it('passes tag parameter to ContextGatherer constructor', async () => {
			...
			expect(ContextGatherer).toHaveBeenCalledWith('/test/project/root', testTag)
		})

		it('passes undefined tag when no tag is provided', async () => {
			await performResearch('Test query', {}, {}, 'json', false)

			expect(ContextGatherer).toHaveBeenCalledWith('/test/project/root', undefined)
		})
	...
	describe('context gathering behavior for different tags', () => {
		...
		it('handles empty task discovery gracefully when readJSON fails', async () => {
			...
			const result = await performResearch('Test query', { tag: 'test-tag' }, {}, 'json', false)
		...
		it('combines provided taskIds with auto-discovered tasks', async () => {
			...
			expect(mockContextGatherer.gather).toHaveBeenCalledWith({
				tasks: expect.arrayContaining([...providedTaskIds, ...autoDiscoveredIds]),
				files: [],
				customContext: '',
				includeProjectTree: false,
				format: 'research',
	...
	describe('tag-aware fuzzy search', () => {
		it('initializes FuzzyTaskSearch with flattened tasks from correct tag', async () => {
			...
			expect(FuzzyTaskSearch).toHaveBeenCalledWith(mockFlattenedTasks, 'research')
		})

		it('calls fuzzy search with correct parameters', async () => {
			const testQuery = 'authentication implementation'

			await performResearch(testQuery, { tag: 'feature-branch' }, {}, 'json', false)

			expect(mockFuzzySearch.findRelevantTasks).toHaveBeenCalledWith(testQuery, {
				maxResults: 8,
				includeRecent: true,
		...
		it('handles empty tasks data gracefully', async () => {
			...
			await performResearch('Test query', { tag: 'empty-tag' }, {}, 'json', false)
		...
		it('handles null tasks data gracefully', async () => {
			...
			await performResearch('Test query', { tag: 'null-tag' }, {}, 'json', false)
	...
	describe('error handling for invalid tags', () => {
		it('continues execution when readJSON throws error for invalid tag', async () => {
			...
			const result = await performResearch('Test query', { tag: 'invalid-tag' }, {}, 'json', false)
		...
		it('logs debug message when task discovery fails', async () => {
			...
			await performResearch('Test query', { tag: 'error-tag' }, { mcpLog: mockLog }, 'json', false)
		...
		it('handles ContextGatherer.gather errors gracefully', async () => {
			mockContextGatherer.gather.mockRejectedValueOnce(new Error('Gather failed'))

			await expect(
				performResearch('Test query', { tag: 'gather-error-tag' }, {}, 'json', false)
	...
	describe('MCP integration with tags', () => {
		it('uses MCP logger when mcpLog is provided in context', async () => {
			...
			await performResearch('Test query', { tag: 'mcp-tag' }, { mcpLog: mockMCPLog }, 'json', false)
	...
	describe('output format handling with tags', () => {
		it('displays UI banner only in text format', async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('🔍 AI Research Query'))
		...
		it('does not display UI banner in json format', async () => {
			...
			expect(consoleSpy).not.toHaveBeenCalledWith(expect.stringContaining('🔍 AI Research Query'))
	...
	describe('comprehensive tag integration test', () => {
		it('performs complete research flow with tag-aware functionality', async () => {
			...
			expect(ContextGatherer).toHaveBeenCalledWith('/custom/root', 'integration-test')
