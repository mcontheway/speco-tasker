...
describe('任务导出功能验证', () => {
	...
	describe('导出数据结构验证', () => {
		...
		it('应该支持不同导出格式的元数据', () => {
			...
			formats.forEach(formatInfo => {
				...
				expect(exportData.metadata.fileExtension).toBe(formatInfo.fileExtension)
			})
		})
		...
		it('应该正确统计导出数据的统计信息', () => {
			...
			const totalSubtasks = mockTasksData.master.tasks.reduce((sum, task) => sum + task.subtasks.length, 0)
	...
	describe('导出文件操作验证', () => {
		it('应该能够生成正确的导出文件名', () => {
			...
			expect(featureFileName).toBe('tasks_export_2024-01-01_feature-branch.json')
		})
		...
		it('应该验证导出文件的完整性检查', () => {
			...
			expect(parsedData.tasks.every(task => task.id && task.title)).toBe(true)
	...
	describe('导出配置和选项验证', () => {
		...
		it('应该验证导出选项的边界情况', () => {
			...
			// 验证无效选项应该被处理
			expect(['json', 'csv', 'xml', 'markdown']).not.toContain(invalidOptions.format)
