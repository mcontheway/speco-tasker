		...
		it("应该验证必需的初始化参数", () => {
			const validateInitParams = (params) => {
				...
				if (
					...
					!/^\d+\.\d+\.\d+$/.test(params.projectVersion)
				) {
			...
			// 有效的参数
			expect(validateInitParams({ projectName: "test-project" }).valid).toBe(
				true,
			);
			expect(
				validateInitParams({
			...
			expect(validateInitParams({ projectName: "a".repeat(60) }).valid).toBe(
				false,
			);
			expect(
				validateInitParams({ projectName: "test", projectVersion: "invalid" })
					.valid,
			).toBe(false);
		});
	...
	describe("目录结构创建", () => {
		...
		it("应该处理目录创建失败的情况", () => {
			...
			const createProjectStructure = (projectRoot) => {
				try {
					fs.mkdirSync(path.join(projectRoot, ".taskmaster"), {
						recursive: true,
					});
					return { success: true };
				} catch (error) {
		...
		it("应该创建示例任务模板", () => {
			const createExampleTasks = (projectRoot) => {
				...
				const examplePath = path.join(
					projectRoot,
					".taskmaster/templates/example-tasks.json",
				);
		...
		it("应该执行完整的项目初始化", () => {
			...
			const initializeProject = (params) => {
				...
				try {
					...
					// 2. 创建目录结构
					steps.push("创建目录结构");
					fs.mkdirSync(path.join(params.projectRoot, ".taskmaster"), {
						recursive: true,
					});
					...
					fs.writeFileSync(
						path.join(params.projectRoot, ".taskmaster/config.json"),
					...
					const tasks = { tasks: [] };
					fs.writeFileSync(
						path.join(params.projectRoot, ".taskmaster/tasks/tasks.json"),
					...
					if (params.createReadme) {
						...
						fs.writeFileSync(
							path.join(params.projectRoot, "README.md"),
							readme,
						);
					}
