...
describe("任务排序功能验证", () => {
	...
	describe("按状态排序功能", () => {
		it("应该能够按状态排序任务", () => {
			...
			expect(sortedTasks.map((t) => t.status)).toEqual([
				"pending",
				"in-progress",
				"blocked",
				"done",
	...
	describe("按时间排序功能", () => {
		...
		it("应该处理无效的时间格式", () => {
			...
			const sortedTasks = tasks.sort((a, b) => {
				...
				return (
					(isNaN(aTime) ? Number.POSITIVE_INFINITY : aTime) -
					(isNaN(bTime) ? Number.POSITIVE_INFINITY : bTime)
				);
			});
	...
	describe("排序稳定性验证", () => {
		it("应该保持排序的稳定性", () => {
			...
			expect(sortedTasks1.map((t) => t.id)).toEqual(
				sortedTasks2.map((t) => t.id),
			);
		});
		...
		it("应该正确处理空数组和单个元素", () => {
			...
			expect(singleTask.sort((a, b) => a.id - b.id)).toEqual([
				{ id: 1, title: "单个任务" },
			]);
		});
	});
