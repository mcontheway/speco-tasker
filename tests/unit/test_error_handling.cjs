...
describe('错误处理功能验证', () => {
	...
	describe('错误分类和类型', () => {
		it('应该能够分类不同类型的错误', () => {
			const categorizeError = (error) => {
				...
				if (error.message && error.message.includes('JSON')) return 'INVALID_JSON'
				if (error.message && error.message.toLowerCase().includes('network')) return 'NETWORK_ERROR'
				if (error.message && error.message.includes('timeout')) return 'TIMEOUT_ERROR'
				return 'UNKNOWN_ERROR'
			}

			expect(categorizeError({ code: 'ENOENT' })).toBe('FILE_NOT_FOUND')
			expect(categorizeError({ code: 'EACCES' })).toBe('PERMISSION_DENIED')
			expect(categorizeError({ message: 'Invalid JSON format' })).toBe('INVALID_JSON')
			expect(categorizeError({ message: 'Network connection failed' })).toBe('NETWORK_ERROR')
			expect(categorizeError({ message: 'Request timeout' })).toBe('TIMEOUT_ERROR')
			expect(categorizeError({ message: 'Unknown error' })).toBe('UNKNOWN_ERROR')
		})
		...
		it('应该提供用户友好的错误信息', () => {
			const getUserFriendlyMessage = (errorType, originalError) => {
				...
				const details = (originalError && originalError.message) ? ` (详情: ${originalError.message})` : ''

				return baseMessage + details
			}

			expect(getUserFriendlyMessage('FILE_NOT_FOUND')).toContain('文件或目录不存在')
			const jsonErrorMessage = getUserFriendlyMessage('INVALID_JSON', { message: 'Unexpected token' })
			expect(jsonErrorMessage).toContain('不是有效的JSON格式')
			expect(jsonErrorMessage).toContain('Unexpected token')
			expect(getUserFriendlyMessage('UNKNOWN_ERROR')).toContain('发生了未知错误')
		})

		it('应该根据错误严重程度进行分级', () => {
			const getErrorSeverity = (errorType) => {
				const criticalErrors = ['NETWORK_ERROR', 'PERMISSION_DENIED', 'UNKNOWN_ERROR']
	...
	describe('异常处理机制', () => {
		...
		it('应该能够处理异步异常', async () => {
			...
			const failedResult = await safeExecuteAsync(riskyAsyncFunction, 'async default')
		...
		it('应该支持错误重试机制', async () => {
			...
			const result = await retryExecute(() => Promise.resolve(unreliableFunction()))
		...
		it('应该支持降级处理策略', () => {
			const executeWithFallback = (primaryFn, fallbackFn) => {
				try {
					...
					try {
						return { success: true, result: fallbackFn(), strategy: 'fallback' }
					} catch (fallbackError) {
						return {
							success: false,
	...
	describe('错误日志和监控', () => {
		it('应该能够记录错误详情', () => {
			...
			const loggedError = logError(testError, { userId: 'user123', action: 'file_upload' })
		...
		it('应该支持错误聚合和统计', () => {
			...
			recordError(new Error('Network timeout'), { url: 'http://api.example.com' })
		...
		it('应该支持错误警报机制', () => {
			...
			expect(currentAlerts.some(alert => alert.type === 'ERROR_COUNT_THRESHOLD')).toBe(true)
			expect(currentAlerts.some(alert => alert.type === 'ERROR_RATE_THRESHOLD')).toBe(true)
		})
	})
	...
	describe('错误恢复和重试策略', () => {
		it('应该实现指数退避重试策略', async () => {
			...
			const exponentialBackoffRetry = async (operation, maxRetries, baseDelay) => {
				let lastError

				for (let attempt = 0; attempt <= maxRetries; attempt++) {
					try {
						return { success: true, result: await operation(), attempts: attempt + 1 }
					} catch (error) {
						lastError = error
					...
					executeWithDegradation: async (operation) => {
						...
						for (const service of availableServices) {
							try {
								...
								// 如果还有其他服务可用，继续尝试
								if (availableServices.indexOf(service) < availableServices.length - 1) {
									continue
								}
							}
						}
