...
describe('任务搜索功能验证', () => {
	...
	describe('基本搜索功能', () => {
		it('应该支持按标题搜索任务', () => {
			...
			// 搜索不存在的标题
			const nonExistentResults = searchTasks(mockTasks, '不存在的任务', 'title')
			...
			const searchTasks = (tasks, query, field = 'all') => {
				...
				return tasks.filter(task => {
					return keywords.every(keyword => {
						...
						if (field === 'description' || field === 'all') {
							if (task.description && task.description.toLowerCase().includes(keyword)) {
								return true
							}
						}

						if (field === 'tags' || field === 'all') {
							if (task.tags && task.tags.some(tag => tag.toLowerCase().includes(keyword))) {
								return true
							}
						}
	...
	describe('搜索结果处理', () => {
		...
		it('应该支持分页搜索结果', () => {
			...
			const paginatedResult = searchTasksWithPagination(mockTasks, '用户', { page: 1, limit: 1 })
		...
		it('应该支持按相关度排序搜索结果', () => {
			const searchTasksWithRelevance = (tasks, query, field = 'all') => {
				...
				const scoredResults = tasks.map(task => {
					let score = 0
					const matches = []
			...
			expect(relevanceResults[0].relevanceScore).toBeGreaterThanOrEqual(relevanceResults[1].relevanceScore)
