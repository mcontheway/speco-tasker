...
describe('Tag Management – writeJSON context preservation', () => {
	...
	it('renameTag should keep overall structure intact', async () => {
		await createTag(TASKS_PATH, 'oldtag', {}, { projectRoot: TEMP_DIR }, 'json')

		await renameTag(TASKS_PATH, 'oldtag', 'newtag', {}, { projectRoot: TEMP_DIR }, 'json')
	...
	it('copyTag then deleteTag preserves other tags', async () => {
		await createTag(TASKS_PATH, 'source', {}, { projectRoot: TEMP_DIR }, 'json')

		await copyTag(TASKS_PATH, 'source', 'copy', {}, { projectRoot: TEMP_DIR }, 'json')

		await deleteTag(TASKS_PATH, 'copy', { yes: true }, { projectRoot: TEMP_DIR }, 'json')
