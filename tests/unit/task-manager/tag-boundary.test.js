...
describe('Tag boundary resolution', () => {
	...
	it('switches currentTag in state.json when useTag succeeds', async () => {
		await createTag(TASKS_PATH, 'feature-x', {}, { projectRoot: TEMP_DIR }, 'json')
		await useTag(TASKS_PATH, 'feature-x', {}, { projectRoot: TEMP_DIR }, 'json')
	...
	it('copyTag deep copy – mutation does not affect source', async () => {
		...
		await copyTag(TASKS_PATH, 'source', 'clone', {}, { projectRoot: TEMP_DIR }, 'json')
	...
	it('adds task to tag derived from state.json when no explicit tag supplied', async () => {
		...
		await createTag(TASKS_PATH, 'feature-auto', {}, { projectRoot: TEMP_DIR }, 'json')
