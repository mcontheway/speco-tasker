...
describe("任务历史功能验证", () => {
	...
	describe("历史记录数据结构验证", () => {
		...
		it("应该支持历史记录的标签系统集成", () => {
			...
			expect(taggedHistory.details).toContain(
				"标签从 master 变更为 feature-branch",
			);
		});
	});
	...
	describe("历史记录查询功能验证", () => {
		it("应该能够按任务ID查询历史记录", () => {
			...
			// 模拟按任务ID查询
			const task1History = mockHistoryData.filter(
				(entry) => entry.taskId === 1,
			);
			const task2History = mockHistoryData.filter(
				(entry) => entry.taskId === 2,
			);
		...
		it("应该支持按操作类型查询历史记录", () => {
			...
			const actionHistory = mockHistoryData.filter(
				(entry) => entry.action === actionType,
			);

			expect(actionHistory).toHaveLength(2);
			expect(actionHistory.every((entry) => entry.action === actionType)).toBe(
				true,
			);
