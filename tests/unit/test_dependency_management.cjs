...
describe('依赖关系管理功能验证', () => {
	...
	describe('依赖关系错误处理', () => {
		it('应该处理无效的依赖ID类型', () => {
			...
			const validateDependencyIds = (dependencies) => {
				return dependencies.every(dep =>
					typeof dep === 'number' && dep > 0 && Number.isInteger(dep)
				)
			}
		...
		it('应该处理依赖关系中的空值', () => {
			...
			const cleanDependencies = taskWithNullDeps.dependencies.filter(dep =>
		...
		it('应该检测依赖关系中的孤立任务', () => {
			...
			const getIsolatedTasks = (tasks) => {
				...
				return tasks.filter(task => !dependentTasks.has(task.id) && task.dependencies.length === 0)
			}
