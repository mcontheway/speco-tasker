...
describe("文件操作功能验证", () => {
	...
	describe("路径操作", () => {
		it("应该能够解析和操作文件路径", () => {
			...
			expect(joinPaths("project", "src", "index.js")).toBe(
		...
		it("应该能够处理相对路径和绝对路径", () => {
			...
			expect(resolvePath("/base", "relative/file.txt")).toMatch(
	...
	describe("目录操作", () => {
		...
		it("应该能够删除目录", () => {
			...
			expect(result).toBe(true);
			expect(fs.rmSync).toHaveBeenCalledWith("/old/directory", {
				recursive: true,
				force: true,
			});
		});
	...
	describe("文件权限和属性", () => {
		...
		it("应该能够获取文件属性", () => {
			...
			const getFileAttributes = (filePath) => {
				try {
					...
					return {
						...
						permissions: (stats.mode & Number.parseInt("777", 8)).toString(8),
					};
				} catch (error) {
			...
			expect(attributes.accessedAt).toEqual(
		...
		it("应该能够设置文件权限", () => {
			...
			const result = setFilePermissions(
				"/test/file.txt",
				Number.parseInt("755", 8),
			);
			expect(result).toBe(true);
	...
	describe("批量文件操作", () => {
		...
		it("应该支持条件文件操作", () => {
			...
			const results = conditionalDeleteFiles(
				files,
		...
		it("应该提供详细的文件操作日志", () => {
			...
			const loggedReadFile = (filePath) => {
				try {
					...
					const content = fs.readFileSync(filePath, "utf8");
					logFileOperation("READ_SUCCESS", filePath, "success", {
						size: content.length,
					});
					return content;
				} catch (error) {
					logFileOperation("READ_ERROR", filePath, "error", {
						error: error.message,
					});
					throw error;
				}
