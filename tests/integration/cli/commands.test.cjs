...
describe('CLI Models Command (Action Handler Test)', () => {
		...
		try {
			if (options.setMain) {
				...
				if (typeof modelId !== 'string' || modelId.trim() === '') {
					console.error(chalk.red('Error: --set-main flag requires a valid model ID.'))
					process.exit(1)
				}
				const provider = findProvider(modelId)
				if (!provider) {
					console.error(chalk.red(`Error: Model ID "${modelId}" not found in available models.`))
					process.exit(1)
				}
				if (await configManager.setMainModel(provider, modelId)) {
					console.log(chalk.green(`Main model set to: ${modelId} (Provider: ${provider})`))
					modelSetAction = true
				} else {
			...
			if (options.setResearch) {
				...
				if (typeof modelId !== 'string' || modelId.trim() === '') {
					console.error(chalk.red('Error: --set-research flag requires a valid model ID.'))
					process.exit(1)
				}
				const provider = findProvider(modelId)
				if (!provider) {
				...
				if (await configManager.setResearchModel(provider, modelId)) {
					console.log(chalk.green(`Research model set to: ${modelId} (Provider: ${provider})`))
					modelSetAction = true
				} else {
			...
			if (options.setFallback) {
				...
				if (typeof modelId !== 'string' || modelId.trim() === '') {
					console.error(chalk.red('Error: --set-fallback flag requires a valid model ID.'))
					process.exit(1)
				}
				const provider = findProvider(modelId)
				if (!provider) {
				...
				if (await configManager.setFallbackModel(provider, modelId)) {
					console.log(chalk.green(`Fallback model set to: ${modelId} (Provider: ${provider})`))
					modelSetAction = true
				} else {
			...
			;(ui.log || console.error)(`Error processing models command: ${error.message}`, 'error')
			if (error.stack) {
	...
	it('should show an error if --set-main model ID is not found', async () => {
		await expect(modelsAction({ setMain: 'non-existent-model' })).rejects.toThrow(/process.exit/) // Expect exit call
	...
	it('should call setResearchModel with correct provider and ID', async () => {
		...
		expect(mockSetResearchModel).toHaveBeenCalledWith(expectedProvider, modelId)
		expect(console.log).toHaveBeenCalledWith(
	...
	it('should call setFallbackModel with correct provider and ID', async () => {
		...
		expect(mockSetFallbackModel).toHaveBeenCalledWith(expectedProvider, modelId)
		expect(console.log).toHaveBeenCalledWith(
	...
	it('should call all set*Model functions when all flags are used', async () => {
		...
		expect(mockSetResearchModel).toHaveBeenCalledWith(researchProvider, researchModelId)
		expect(mockSetFallbackModel).toHaveBeenCalledWith(fallbackProvider, fallbackModelId)
	})
