...
describe('Cross-Tag Move CLI Integration', () => {
	...
	async function moveAction(options) {
		...
		if (isCrossTagMove) {
			...
			try {
				...
				await generateTaskFilesModule.default(tasksPath, path.dirname(tasksPath), {
					tag: sourceTag
				})
				await generateTaskFilesModule.default(tasksPath, path.dirname(tasksPath), { tag: toTag })
			} catch (error) {
				console.error(chalk.red(`Error: ${error.message}`))
	...
	it('should fail to move task with cross-tag dependencies', async () => {
		...
		expect(errorMessages.some((msg) => msg.includes('cross-tag dependency conflicts'))).toBe(true)
	...
	it('should provide helpful error messages for dependency conflicts', async () => {
		...
		expect(mockMoveTasksBetweenTags).toHaveBeenCalled()
		expect(
			errorMessages.some((msg) => msg.includes('Cross-tag dependency conflicts detected'))
		).toBe(true)
	...
	it('should handle same tag error correctly', async () => {
		...
		expect(
			errorMessages.some((msg) =>
				msg.includes('Source and target tags are the same ("backlog") but no destination specified')
			)
		).toBe(true)
		expect(logMessages.some((msg) => msg.includes('For within-tag moves'))).toBe(true)
		expect(logMessages.some((msg) => msg.includes('For cross-tag moves'))).toBe(true)
	...
	it('should fail when invalid task ID is provided', async () => {
		...
		expect(errorMessages.some((msg) => msg.includes('Invalid task ID at position 2'))).toBe(true)
	...
	it('should fail when first task ID is invalid', async () => {
		...
		expect(errorMessages.some((msg) => msg.includes('Invalid task ID at position 1'))).toBe(true)
	...
	it('should fail when last task ID is invalid', async () => {
		...
		expect(errorMessages.some((msg) => msg.includes('Invalid task ID at position 3'))).toBe(true)
	...
	it('should fail when --from parameter is missing for cross-tag move', async () => {
		...
		expect(
			errorMessages.some((msg) => msg.includes('--from parameter is required for cross-tag moves'))
		).toBe(true)
	...
	it('should fail when both --from and --to are missing for within-tag move', async () => {
		...
		expect(
			errorMessages.some((msg) =>
				msg.includes('Both --from and --to parameters are required for within-tag moves')
			)
		).toBe(true)
