...
describe('OpenCode Profile Initialization Functionality', () => {
	...
	beforeAll(() => {
		const opencodeJsPath = path.join(process.cwd(), 'src', 'profiles', 'opencode.js')
	...
	test('opencode.js has lifecycle functions for MCP config transformation', () => {
		expect(opencodeProfileContent).toContain('function onPostConvertRulesProfile')
	...
	test('opencode.js uses custom MCP config name', () => {
		...
		expect(opencodeProfileContent).not.toMatch(/mcpConfigName:\s*['"]mcp\.json['"]/)
	})
