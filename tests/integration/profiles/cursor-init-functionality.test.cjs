...
describe('Cursor Profile Initialization Functionality', () => {
	...
	beforeAll(() => {
		const cursorJsPath = path.join(process.cwd(), 'src', 'profiles', 'cursor.js')
	...
	test('cursor.js preserves .mdc extension in both input and output', () => {
		...
		expect(cursorProfile.fileMap['rules/cursor_rules.mdc']).toBe('cursor_rules.mdc')
	...
	test('cursor.js uses standard tool mappings (no tool renaming)', () => {
		...
		expect(cursorProfile.conversionConfig.toolNames.edit_file).toBe('edit_file')
