...
describe('Claude Profile Initialization Functionality', () => {
	...
	beforeAll(() => {
		const claudeJsPath = path.join(process.cwd(), 'src', 'profiles', 'claude.js')
	...
	test('claude.js has correct asset-only profile configuration', () => {
		...
		expect(claudeProfileContent).toContain("'AGENTS.md': '.taskmaster/CLAUDE.md'")
	...
	test('claude.js has lifecycle functions for file management', () => {
		...
		expect(claudeProfileContent).toContain('function onPostConvertRulesProfile')
	})
