...
describe('Roo Profile Initialization Functionality', () => {
	...
	test('roo.js uses factory pattern with correct configuration', () => {
		...
		expect(rooProfileContent).toContain('toolMappings: COMMON_TOOL_MAPPINGS.ROO_STYLE')
	...
	test('roo.js profile ensures Roo directory structure via onAddRulesProfile', () => {
		...
		expect(rooProfileContent).toContain("const sourceDir = path.join(assetsDir, 'roocode');")
		expect(rooProfileContent).toContain('copyRecursiveSync(sourceDir, targetDir);')

		// Check for the specific .roo modes directory handling
		expect(rooProfileContent).toContain("const rooModesDir = path.join(sourceDir, '.roo');")

		// Check for import of ROO_MODES from profiles.js instead of local definition
		expect(rooProfileContent).toContain("import { ROO_MODES } from '../constants/profiles.js';")
	})
