...
describe('Gemini Profile Initialization Functionality', () => {
	...
	beforeAll(() => {
		const geminiJsPath = path.join(process.cwd(), 'src', 'profiles', 'gemini.js')
	...
	test('gemini.js has correct profile configuration', () => {
		...
		expect(geminiProfileContent).toContain("docsUrl: 'github.com/google-gemini/gemini-cli'")
	...
	test('gemini.js has no lifecycle functions', () => {
		...
		expect(geminiProfileContent).not.toContain('function onPostConvertRulesProfile')
	...
	test('gemini.js uses custom MCP config name', () => {
		...
		expect(geminiProfileContent).not.toMatch(/mcpConfigName:\s*['"]mcp\.json['"]/)
	})
