...
describe('Roo Files Inclusion in Package', () => {
	...
	test('roo.js profile contains logic for Roo directory creation and file copying', () => {
		...
		expect(rooJsContent.includes('path.join(rooModesDir, `rules-${mode}`, `${mode}-rules`)')).toBe(
			true
		)
		expect(
			rooJsContent.includes("path.join(targetDir, '.roo', `rules-${mode}`, `${mode}-rules`)")
		).toBe(true)

		// Check for import of ROO_MODES from profiles.js instead of local definition
		expect(rooJsContent.includes("import { ROO_MODES } from '../constants/profiles.js'")).toBe(true)
		...
		const expectedModes = ['architect', 'ask', 'orchestrator', 'code', 'debug', 'test']
