...
describe('VSCode Profile Initialization Functionality', () => {
	...
	beforeAll(() => {
		const vscodeJsPath = path.join(process.cwd(), 'src', 'profiles', 'vscode.js')
	...
	test('vscode.js uses factory pattern with correct configuration', () => {
		...
		expect(vscodeProfileContent).toContain("docsUrl: 'code.visualstudio.com/docs'")
	...
	test('vscode.js configures .mdc to .instructions.md extension mapping', () => {
		...
		expect(vscodeProfile.fileMap['rules/cursor_rules.mdc']).toBe('vscode_rules.instructions.md')
	})
	...
	test('vscode.js uses standard tool mappings', () => {
		...
		expect(vscodeProfile.conversionConfig.toolNames.edit_file).toBe('edit_file')
