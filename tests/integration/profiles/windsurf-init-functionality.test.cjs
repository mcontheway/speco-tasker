...
describe('Windsurf Profile Initialization Functionality', () => {
	...
	beforeAll(() => {
		const windsurfJsPath = path.join(process.cwd(), 'src', 'profiles', 'windsurf.js')
	...
	test('windsurf.js configures .mdc to .md extension mapping', () => {
		...
		expect(windsurfProfile.fileMap['rules/cursor_rules.mdc']).toBe('windsurf_rules.md')
	})
	...
	test('windsurf.js uses standard tool mappings', () => {
		...
		expect(windsurfProfile.conversionConfig.toolNames.edit_file).toBe('edit_file')
