...
function monitorMemoryUsage(
	...
	return new Promise((resolve, reject) => {
		...
		child.stdout.on("data", (data) => {
			...
			if (memoryMatch) {
				const heapUsed = Number.parseInt(memoryMatch[1]);
				samples.push({
			...
			if (
				!monitoring &&
				(output.includes("task-master") ||
					output.includes("TaskMasterMCPServer"))
			) {
				monitoring = true;
				startTime = Date.now();
			}
...
function monitorGarbageCollection(scriptPath, args = [], duration = 5000) {
	return new Promise((resolve, reject) => {
		...
		const child = spawn(
			"node",
			["--expose-gc", "--max-old-space-size=512", scriptPath, ...args],
			{
				stdio: ["pipe", "pipe", "pipe"],
				cwd: path.resolve(__dirname, "../.."),
				env: {
...
describe.skip("Task Master Memory Usage", () => {
	...
	describe("Memory Usage Comparison", () => {
		it("should compare memory usage between CLI and MCP server", async () => {
			...
			console.log(
				`Difference: ${(mcpStats.average - cliStats.average).toFixed(2)} MB`,
			);
	...
	describe("Memory Stability", () => {
		it("should maintain stable memory usage without significant growth", async () => {
			...
			const mean =
				heapUsages.reduce((sum, usage) => sum + usage, 0) / heapUsages.length;
			const variance =
				heapUsages.reduce((sum, usage) => sum + (usage - mean) ** 2, 0) /
				heapUsages.length;
