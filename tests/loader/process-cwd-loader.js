...
// Create a Jest-compatible cwd protection
// (originalCwd already declared at top of file)
const smartCwdDescriptor = {
	get: function () {
		// If Jest has mocked this function, use the mock
		if (this.cwd !== originalCwd && typeof this.cwd === "function") {
			try {
				return this.cwd();
			} catch (error) {
				console.warn(
					"[LOADER] Mock cwd failed, using fallback:",
					error.message,
				);
			}
		}
	...
	set: function (value) {
		// Allow any overrides (including Jest mocks)
		if (typeof value === "function") {
			// Jest mock function
			this.cwd = value;
		} else if (typeof jest !== "undefined") {
			// Allow Jest to set whatever it wants
			this.cwd = value;
		} else {
			// In production, log but allow
			console.warn("[LOADER] process.cwd override in production:", value);
			this.cwd = value;
		}
	},
	enumerable: true,
...
// Let Jest handle process.cwd naturally - don't interfere
console.log("[LOADER] Loader initialized for graceful-fs compatibility");
...
export async function resolve(specifier, context, defaultResolve) {
	...
	if (
		...
		const mockUrl = new URL(
			"./graceful-fs-ultra-safe.mock.js",
