...
class MockProgressReporter {
	...
	printSummary() {
		...
		this.progressHistory.forEach((entry, index) => {
			...
			console.log(`${index + 1}. [${entry.timestamp}ms] ${percentage}% - ${entry.message}`)
		})
		...
		const hasTaskMessages = this.progressHistory.some((entry) =>
			/^[🔴🟠🟢⚪]{3} Task \d+\/\d+ - .+ \| ~Output: \d+ tokens/u.test(entry.message.trim())
		)
		...
		console.log(`✅ Initial message format: ${hasInitialMessage ? 'PASS' : 'FAIL'}`)
		console.log(`✅ Task message format: ${hasTaskMessages ? 'PASS' : 'FAIL'}`)
		console.log(`✅ Completion message format: ${hasCompletionMessage ? 'PASS' : 'FAIL'}`)
	}
}
...
function compareResults(streamingResult, nonStreamingResult) {
	...
	const durationDiff = Math.abs(streamingResult.duration - nonStreamingResult.duration)
	const durationDiffPercent = Math.round(
		(durationDiff / Math.max(streamingResult.duration, nonStreamingResult.duration)) * 100
	)

	console.log(`Duration difference: ${durationDiff}ms (${durationDiffPercent}%)`)

	if (streamingResult.progressHistory) {
		console.log(`Streaming progress reports: ${streamingResult.progressHistory.length}`)
	}
...
async function main() {
	...
	try {
		switch (testType.toLowerCase()) {
			...
			case 'both': {
				console.log(chalk.yellow('Running both MCP streaming and non-streaming tests...\n'))
			...
			case 'all': {
				...
				console.log(`Non-streaming: ${nonStreamResult.success ? '✅ PASS' : '❌ FAIL'}`)
				break
			}
			...
			default:
				...
				console.log(
					chalk.yellow('Available options: mcp-streaming, cli-streaming, non-streaming, both, all')
				)
				process.exit(1)
		}
