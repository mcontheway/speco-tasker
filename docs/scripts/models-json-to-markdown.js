...
const supportedModelsPath = path.join(__dirname, '..', 'modules', 'supported-models.json')
const outputMarkdownPath = path.join(__dirname, '..', '..', 'docs', 'models.md')
...
function main() {
	try {
		...
		const supportedModelsContent = fs.readFileSync(correctSupportedModelsPath, 'utf8')
				...
				models.forEach((model) => {
					...
					if (isSupported) {
						const modelEntry = {
							...
							inputCost: model.cost_per_1m_tokens ? model.cost_per_1m_tokens.input : null,
							outputCost: model.cost_per_1m_tokens ? model.cost_per_1m_tokens.output : null
						}
						if (model.allowed_roles && model.allowed_roles.includes('main')) {
							mainModels.push(modelEntry)
						}
						if (model.allowed_roles && model.allowed_roles.includes('research')) {
							researchModels.push(modelEntry)
						}
						if (model.allowed_roles && model.allowed_roles.includes('fallback')) {
