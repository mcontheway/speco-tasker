...
export class CacheManager {
	...
	get(key: string): any {
		...
		if (cached) {
			...
			if (!isExpired) {
				...
				this.logger.debug(`Cache hit for ${key} (${accessTime}ms, ${cached.accessCount} accesses)`)
				return cached.data
			} else {
	...
	clearPattern(pattern: string): void {
		...
		if (evictedCount > 0) {
			...
			this.logger.debug(`Evicted ${evictedCount} cache entries matching pattern: ${pattern}`)
		}
	}
	...
	private initializeBackgroundRefresh(): void {
		...
		this.logger.debug(`Cache background refresh initialized with ${interval}ms interval`)
	}
	...
	private scheduleRelatedDataPrefetch(key: string, data: any): void {
		if (key.includes('get_tasks') && Array.isArray(data)) {
			this.logger.debug(`Scheduled prefetch for ${data.length} tasks related to ${key}`)
		}
	}
