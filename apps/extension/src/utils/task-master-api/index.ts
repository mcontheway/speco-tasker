...
export class TaskMasterApi {
	...
	constructor(mcpClient: MCPClientManager, config?: Partial<TaskMasterApiConfig>) {
		...
		const mergedCache: CacheConfig = {
			...
			enableBackgroundRefresh:
				config?.cache?.enableBackgroundRefresh ?? this.defaultCacheConfig.enableBackgroundRefresh,
			refreshInterval: config?.cache?.refreshInterval ?? this.defaultCacheConfig.refreshInterval,
			enableAnalytics: config?.cache?.enableAnalytics ?? this.defaultCacheConfig.enableAnalytics,
			enablePrefetch: config?.cache?.enablePrefetch ?? this.defaultCacheConfig.enablePrefetch,
			compressionEnabled:
				config?.cache?.compressionEnabled ?? this.defaultCacheConfig.compressionEnabled,
			persistToDisk: config?.cache?.persistToDisk ?? this.defaultCacheConfig.persistToDisk
		}
	 ...
	 */
	async getTasks(options?: GetTasksOptions): Promise<TaskMasterApiResponse<TaskMasterTask[]>> {
		...
		try {
			...
			// Transform response
			const transformedTasks = this.transformer.transformMCPTasksResponse(mcpResponse)
	...
	private async performBackgroundRefresh(): Promise<void> {
		...
		this.logger.log(`Background refresh completed, refreshed ${refreshedCount} entries`)
	}
