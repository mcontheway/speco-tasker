	...
	async callTool(toolName: string, args: Record<string, unknown>): Promise<any> {
		...
		for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
			try {
				...
				this.logger.debug(`Raw MCP response for ${toolName}:`, JSON.stringify(rawResponse, null, 2))
				...
				if (
					...
					if (contentItem.type === 'text' && contentItem.text) {
						try {
							...
							this.logger.error(`Failed to parse MCP response text for ${toolName}:`, parseError)
				...
				this.logger.warn(`Unexpected MCP response format for ${toolName}, returning raw response`)
				return rawResponse
			} catch (error) {
		...
		throw (
			lastError ||
			new Error(`Failed to call ${toolName} after ${this.config.retryAttempts} attempts`)
		)
	}
