...
export class MCPClientManager {
	...
	constructor(config: MCPConfig) {
		logger.log('🔍 DEBUGGING: MCPClientManager constructor called with config:', config)
		this.config = config
	}
	...
	private async _doConnect(): Promise<void> {
		try {
			...
			logger.log(`Starting MCP server: ${this.config.command} ${this.config.args?.join(' ') || ''}`)
			...
			this.transport.onerror = (error: Error) => {
				...
				vscode.window.showErrorMessage(`TaskMaster MCP transport error: ${error.message}`)
			}
			...
			if (this.transport && (this.transport as any).process) {
				...
				proc.on('exit', (code: number, signal: string) => {
					logger.log(`🔚 MCP server process exited with code ${code}, signal ${signal}`)
					if (code !== 0) {
			...
			try {
				...
				if (versionResult?.content?.[0]?.text) {
					...
					if (response?.version) {
						...
						logger.log(`   Package: ${response.version.name || 'task-master-ai'}`)
						if (response.tag) {
							logger.log(`   Current Tag: ${response.tag.currentTag || 'master'}`)
						}
						logger.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
					}
				}
	...
	async testConnection(): Promise<boolean> {
		try {
			...
			logger.log('Available MCP tools:', result.tools?.map((t) => t.name) || [])
...
export function createMCPConfigFromSettings(): MCPConfig {
	logger.log('🔍 DEBUGGING: createMCPConfigFromSettings called at', new Date().toISOString())
	...
	if (command === 'node' && args.length === 0) {
		try {
			...
			const mcpServerPath = path.resolve(path.dirname(taskMasterPath), 'mcp-server/server.js')
