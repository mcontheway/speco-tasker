...
export class ErrorHandler {
	...
	private notifyUser(context: ErrorContext): void {
		...
		switch (context.severity) {
			...
			case ErrorSeverity.MEDIUM:
				...
				if ([ErrorCategory.CONFIGURATION, ErrorCategory.TASK_LOADING].includes(context.category)) {
					if (actions.length > 0) {
						vscode.window
						...
						vscode.window.showInformationMessage(`TaskMaster: ${context.message}`)
					}
				}
				break
		}
	...
	private generateIssueUrl(context: ErrorContext): string {
		...
		return `https://github.com/mcontheway/taskmaster-no-ai/issues/new?title=${title}&body=${body}`
	}
