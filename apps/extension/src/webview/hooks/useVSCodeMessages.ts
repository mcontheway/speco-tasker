		...
		const handleMessage = (event: MessageEvent) => {
			...
			if (message.requestId) {
				...
				if (pending) {
					...
					if (message.type === 'response') {
						...
						const isSuccess =
							message.success !== undefined ? message.success : message.data !== undefined
						if (isSuccess) {
			...
			switch (message.type) {
				...
				case 'pollingUpdate':
					...
					if (!state.polling.isUserInteracting && !state.polling.isOfflineMode) {
						dispatch({
...
function handleErrorNotification(message: any, dispatch: React.Dispatch<AppAction>) {
