...
export const useTaskDetails = ({ taskId, sendMessage, tasks }: UseTaskDetailsProps) => {
	...
	const { data: fullTaskData, error: taskDetailsError } = useTaskDetailsQuery(taskIdForFetch)
	...
	const mergedCurrentTask = useMemo(() => {
		...
		if (isSubtask && fullTaskData.subtasks) {
			...
			const subtaskData = fullTaskData.subtasks.find(
				(st: any) => st.id === currentTask.id || st.id === parseInt(currentTask.id as any)
			)
			if (subtaskData) {
