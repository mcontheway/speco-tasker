		...
		try {
			...
			config = {
				models: {
					...
					fallback:
						parsedConfig?.models?.fallback?.provider && parsedConfig?.models?.fallback?.modelId
			...
			if (config.models.fallback?.provider && !validateProvider(config.models.fallback.provider)) {
				console.warn(
					chalk.yellow(
...
function validateClaudeCodeSettings(settings) {
	...
	const CommandSpecificSchema = z.record(z.enum(AI_COMMAND_NAMES), BaseSettingsSchema)
...
function getModelConfigForRole(role, explicitRoot = null) {
	...
	if (!roleConfig) {
		log('warn', `No model configuration found for role: ${role}. Returning default.`)
...
function getMcpApiKeyStatus(providerName, projectRoot = null) {
	...
	try {
		...
		console.error(chalk.red(`Error reading or parsing .cursor/mcp.json: ${error.message}`))
		return false
	}
}
		...
		if (models.length > 0) {
			models
				...
				.forEach((modelObj) => {
					...
					const nameParts = modelId.split('-').map((p) => p.charAt(0).toUpperCase() + p.slice(1))
					// Handle specific known names better if needed
					let name = nameParts.join(' ')
					if (modelId === 'claude-3.5-sonnet-20240620') name = 'Claude 3.5 Sonnet'
					if (modelId === 'claude-3-7-sonnet-20250219') name = 'Claude 3.7 Sonnet'
