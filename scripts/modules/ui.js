	...
	console.log(
		boxen(
			chalk.white.bold('FYI: ') +
				...
				chalk.cyan('Use the --tag flag to create/read/update/filter tasks by tag.'),
			{
	...
	const formattedDeps = dependencies.map((depId) => {
		...
		if (depIdStr.includes('.')) {
			...
			if (parts.length !== 2 || !parts[0] || !parts[1]) {
				...
				if (!depTask) {
					return forConsole ? chalk.red(`${depIdStr} (Not found)`) : `${depIdStr} (Not found)`
				}
...
function displayHelp() {
	...
	const configValueWidth = Math.max(30, Math.floor(configTerminalWidth * 0.3) - 10)
 ...
 */
async function displayNextTask(tasksPath, complexityReportPath = null, context = {}) {
	...
	taskTable.push(
		...
		[
			...
			formatDependenciesWithStatus(nextTask.dependencies, data.tasks, true, complexityReport)
		],
		[
	...
	if (nextTask.details && nextTask.details.trim().length > 0) {
		console.log(
			boxen(chalk.white.bold('Implementation Details:') + '\n\n' + nextTask.details, {
				padding: { top: 0, bottom: 0, left: 1, right: 1 },
				borderColor: 'cyan',
				borderStyle: 'round',
				margin: { top: 1, bottom: 0 }
	...
	if (!isSubtask && nextTask.subtasks && nextTask.subtasks.length > 0) {
		...
		nextTask.subtasks.forEach((st) => {
			...
			if (st.dependencies && st.dependencies.length > 0) {
				...
				subtaskDeps =
					formattedDeps.length === 1 ? formattedDeps[0] : formattedDeps.join(chalk.white(', '))
			}
...
async function displayTaskById(
	...
	if (task.isSubtask || task.parentTask) {
		console.log(
			boxen(chalk.white.bold(`Subtask: #${task.parentTask.id}.${task.id} - ${task.title}`), {
				padding: { top: 0, bottom: 0, left: 1, right: 1 },
				borderColor: 'magenta',
				borderStyle: 'round',
				margin: { top: 1, bottom: 0 }
	...
	taskTable.push(
		...
		[
			chalk.cyan.bold('Dependencies:'),
			formatDependenciesWithStatus(task.dependencies, data.tasks, true, complexityReport)
		],
		[
			chalk.cyan.bold('Complexity:'),
			task.complexityScore ? getComplexityWithColor(task.complexityScore) : chalk.gray('N/A')
	...
	if (task.subtasks && task.subtasks.length > 0) {
		...
		task.subtasks.forEach((st) => {
			...
			if (st.dependencies && st.dependencies.length > 0) {
				const formattedDeps = st.dependencies.map((depId) => {
					...
					if (foundDepSubtask) {
						const isDone =
							foundDepSubtask.status === 'done' || foundDepSubtask.status === 'completed'
		...
		console.log(
			boxen(
				chalk.yellow(`No subtasks found matching status: ${statusFilter} (Task has no subtasks)`),
				{
					padding: { top: 0, bottom: 0, left: 1, right: 1 },
					margin: { top: 1, bottom: 0 },
					borderColor: 'yellow',
	...
	} else if (statusFilter && originalSubtaskCount > 0 && task.subtasks.length === 0) {
	...
	} else if (!statusFilter && (!originalSubtasks || originalSubtasks.length === 0)) {
	...
	if (subtasksForProgress && subtasksForProgress.length > 0) {
		...
		const pendingSubtasks = subtasksForProgress.filter((st) => st.status === 'pending').length
		const blockedSubtasks = subtasksForProgress.filter((st) => st.status === 'blocked').length
		const deferredSubtasks = subtasksForProgress.filter((st) => st.status === 'deferred').length
		const cancelledSubtasks = subtasksForProgress.filter((st) => st.status === 'cancelled').length
		...
		const progressBarLength = Math.max(
			20,
			Math.min(60, availableWidth - boxPadding - boxBorders - percentTextLength - 35)
		)
...
function displayApiKeyStatus(statusReport) {
	...
	const table = new Table({
		head: [chalk.cyan('Provider'), chalk.cyan('CLI Key (.env)'), chalk.cyan('MCP Key (mcp.json)')],
		colWidths: [15, 20, 25],
		chars: { mid: '', 'left-mid': '', 'mid-mid': '', 'right-mid': '' }
	})
...
const formatSweScoreWithTertileStars = (score, allModels) => {
	...
	if (n > 0) {
		...
		else if (score >= sortedScores[midThirdIndex]) stars = chalk.yellow('★★') + chalk.gray('☆')
...
function displayModelConfiguration(configData, allAvailableModels = []) {
	...
	activeTable.push([
		...
		formatSweScoreWithTertileStars(active.research.sweScore, allAvailableModels),
	...
	if (active.fallback && active.fallback.provider && active.fallback.modelId) {
		activeTable.push([
			...
			formatSweScoreWithTertileStars(active.fallback.sweScore, allAvailableModels),
...
function displayAiUsageSummary(telemetryData, outputType = 'cli') {
	if ((outputType !== 'cli' && outputType !== 'text') || !telemetryData || isSilentMode()) {
...
async function displayMultipleTasksSummary(
	...
	taskIds.forEach((id) => {
		const { task } = findTaskById(data.tasks, id, complexityReport, statusFilter)
		if (task) {
	...
	foundTasks.forEach((task) => {
		...
		if (task.subtasks && task.subtasks.length > 0) {
			...
			if (inProgress > 0) subtaskSummary += ` ${chalk.hex('#FFA500')(`+${inProgress}`)}`
			...
			progressBar = createProgressBar(completionPercentage, barLength, statusBreakdown)
		}
		...
		if (choice.toLowerCase() === 'q') {
			...
			switch (actionChoice.trim()) {
				case '1':
					...
					console.log(chalk.green('✓ Copy and run this command to mark all tasks as in-progress'))
					break
				case '2':
					console.log(
						chalk.blue(`\n→ Command: task-master set-status --id=${taskIdList} --status=done`)
					)
					console.log(chalk.green('✓ Copy and run this command to mark all tasks as done'))
					break
				case '3':
				...
				case '5': {
					...
					foundTasks.forEach((task) => {
						if (task.dependencies && task.dependencies.length > 0) {
							console.log(chalk.cyan(`Task ${task.id} depends on: ${task.dependencies.join(', ')}`))
				...
				case '6':
					...
					console.log(chalk.green('✓ Copy and run this command to generate task files'))
					break
				default:
			...
			// Show specific task
			await displayTaskById(tasksPath, choice.trim(), complexityReportPath, statusFilter, context)
		}
	} else {
...
function displayContextAnalysis(analysisData, semanticQuery, contextSize) {
	...
	const { highRelevance, mediumRelevance, recentTasks, allRelevantTasks } = analysisData
	...
	analysisContent +=
		chalk.gray('Context size: ') + chalk.cyan(`${contextSize.toLocaleString()} characters`) + '\n'
	analysisContent +=
		chalk.gray('Tasks found: ') + chalk.yellow(`${allRelevantTasks.length} relevant tasks`) + '\n\n'
	...
	if (highRelevance.length > 0) {
		...
		if (highRelevance.length > 3) {
			analysisContent +=
				chalk.green(`  • ... and ${highRelevance.length - 3} more high relevance tasks`) + '\n'
		}
		analysisContent += '\n'
	}
	...
	if (mediumRelevance.length > 0) {
		...
		if (mediumRelevance.length > 3) {
			analysisContent +=
				chalk.yellow(`  • ... and ${mediumRelevance.length - 3} more medium relevance tasks`) + '\n'
		}
		analysisContent += '\n'
	}
	...
	if (recentTasksNotInRelevance.length > 0) {
		...
		if (recentTasksNotInRelevance.length > 2) {
			analysisContent +=
				chalk.cyan(`  • ... and ${recentTasksNotInRelevance.length - 2} more recent tasks`) + '\n'
		}
	}
 ...
 */
export function displayCrossTagDependencyError(conflicts, sourceTag, targetTag, sourceIds) {
 ...
 */
export function displayInvalidTagCombinationError(sourceTag, targetTag, reason) {
	...
	console.log(`  2. Use within-tag move: task-master move --from=<id> --to=<id> --tag=${sourceTag}`)
