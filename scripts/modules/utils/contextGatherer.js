...
export class ContextGatherer {
	...
	_loadAllTasks() {
		try {
			...
			console.warn(`Warning: Could not load tasks for ContextGatherer: ${error.message}`)
			return []
		}
	}
	...
	async gather(options = {}) {
		...
		if (semanticQuery && this.allTasks.length > 0) {
			const semanticResults = this._performSemanticSearch(semanticQuery, maxSemanticResults)
		...
		if (dependencyTasks.length > 0) {
			...
			dependencyResults.allRelatedTaskIds.forEach((id) => finalTaskIds.add(String(id)))
			// We can format and add dependencyResults.graphVisualization later if needed
		}

		// Add custom context first
		if (customContext && customContext.trim()) {
			const formattedCustomContext = this._formatCustomContext(customContext, format)
		...
		if (finalTaskIds.size > 0) {
			...
			if (taskContextResult.context) {
				...
				if (includeTokenCounts && taskContextResult.breakdown) {
					...
					const taskTokens = taskContextResult.breakdown.reduce((sum, task) => sum + task.tokens, 0)
		...
		// Add file context
		if (files.length > 0) {
			const fileContextResult = await this._gatherFileContext(files, format, includeTokenCounts)
			if (fileContextResult.context) {
				...
				if (includeTokenCounts && fileContextResult.breakdown) {
					...
					const fileTokens = fileContextResult.breakdown.reduce((sum, file) => sum + file.tokens, 0)
		...
		if (includeProjectTree) {
			const treeContextResult = await this._gatherProjectTreeContext(format, includeTokenCounts)
	...
	_performSemanticSearch(query, maxResults) {
		...
		const recentTasks = [...this.allTasks].sort((a, b) => b.id - a.id).slice(0, 5)
	...
	_buildDependencyContext(taskIds) {
		const { allRelatedTaskIds, graphs, depthMap } = this._buildDependencyGraphs(taskIds)
		...
		for (const depTask of uniqueDetailedTasks) {
			const isDirect = taskIds.includes(depTask.id) ? ' [DIRECT DEPENDENCY]' : ''
			...
			if (depTask.details) {
				context += `Implementation Details: ${truncate(depTask.details, 400)}\n`
			}
		}

		if (graphs.length > 0) {
			context += '\n\nDependency Chain Visualization:'
			context += graphs.map((graph) => this._formatDependencyChain(graph)).join('')
		}
	...
	_buildDependencyGraph(taskId, visited, depthMap, depth = 0) {
		...
		const dependencies =
			task.dependencies
				?.map((depId) => this._buildDependencyGraph(depId, visited, depthMap, depth + 1))
				.filter(Boolean) || []
			...
			for (const parsed of parsedIds) {
				...
				if (parsed.type === 'task') {
					...
					if (parentResult.task && parentResult.task.subtasks) {
						const subtask = parentResult.task.subtasks.find((st) => st.id === parsed.subtaskId)
						if (subtask) {
							formattedItem = this._formatSubtaskForContext(subtask, parentResult.task, format)
							itemInfo = {
								id: parsed.fullId,
								type: 'subtask',
								title: subtask.title,
	...
	_formatSubtaskForContext(subtask, parentTask, format) {
		...
		sections.push(`**Subtask ${parentTask.id}.${subtask.id}: ${subtask.title}**`)
	...
	async _gatherFileContext(filePaths, format, includeTokenCounts = false) {
		...
		for (const filePath of filePaths) {
			try {
				...
				if (includeTokenCounts) {
					const formattedFile = this._formatSingleFileForContext(fileData, format)
					breakdown.push({
						path: relativePath,
	...
	async _gatherProjectTreeContext(format, includeTokenCounts = false) {
		try {
			...
			console.warn(`Warning: Could not generate project tree: ${error.message}`)
	...
	_generateFileTree(dirPath, maxDepth, currentDepth = 0) {
		const ignoreDirs = ['.git', 'node_modules', '.env', 'coverage', 'dist', 'build']
			...
			for (const item of items) {
				...
				if (stats.isDirectory()) {
					...
					if (currentDepth < maxDepth - 1) {
						const subtree = this._generateFileTree(itemPath, maxDepth, currentDepth + 1)
						if (subtree) {
