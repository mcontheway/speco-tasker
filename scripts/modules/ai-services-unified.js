...
import { findProjectRoot, getCurrentTag, log, resolveEnvVariable } from './utils.js'
...
function _calculateCost(inputTokens, outputTokens, inputCost, outputCost) {
	const calculatedCost =
		((inputTokens || 0) / 1_000_000) * inputCost + ((outputTokens || 0) / 1_000_000) * outputCost
...
function _readAvailableTags(projectRoot) {
	...
	try {
		...
		const potentialTags = Object.keys(tasksData).filter((key) => _isValidTaggedTask(tasksData[key]))

		return potentialTags.length > 0 ? potentialTags : DEFAULT_TAGS
	} catch (readError) {
		if (getDebugFlag()) {
			log('debug', `Could not read tasks file for available tags: ${readError.message}`)
		}
		return DEFAULT_TAGS
	}
}
...
function _isValidTaggedTask(taskData) {
	return taskData && typeof taskData === 'object' && Array.isArray(taskData.tasks)
}
...
function _getVertexConfiguration(projectRoot, session) {
	const projectId =
		getVertexProjectId(projectRoot) || resolveEnvVariable('VERTEX_PROJECT_ID', session, projectRoot)
	...
	const credentialsPath = resolveEnvVariable('GOOGLE_APPLICATION_CREDENTIALS', session, projectRoot)

	log('debug', `Using Vertex AI configuration: Project ID=${projectId}, Location=${location}`)
...
async function _attemptProviderCallWithRetries(
	...
	throw new Error(`Exhausted all retries for role ${attemptRole} (${fnName} / ${providerName})`)
}
...
async function _unifiedServiceRunner(serviceType, params) {
	...
	for (const currentRole of sequence) {
		...
		try {
			...
			const roleConfig = _getRoleConfiguration(currentRole, effectiveProjectRoot)
			if (!roleConfig) {
				log('error', `Unknown role encountered in _unifiedServiceRunner: ${currentRole}`)
				lastError = lastError || new Error(`Unknown AI role specified: ${currentRole}`)
				continue
			}
			...
			if (!providerName || !modelId) {
				log('warn', `Skipping role '${currentRole}': Provider or Model ID not configured.`)
				lastError =
					lastError ||
					new Error(
			...
			if (!provider) {
				log('warn', `Skipping role '${currentRole}': Provider '${providerName}' not supported.`)
				lastError = lastError || new Error(`Unsupported provider configured: ${providerName}`)
				continue
			}
			...
			if (providerName?.toLowerCase() === 'vertex') {
				providerSpecificParams = _getVertexConfiguration(effectiveProjectRoot, session)
			}
			...
			const callParams = {
				...
				messages,
				...(baseURL && { baseURL }),
				...((serviceType === 'generateObject' || serviceType === 'streamObject') && {
					schema,
					objectName
				}),
			...
			} else if (serviceType === 'streamText' || serviceType === 'streamObject') {
				finalMainResult = providerResponse
			} else {
				log('error', `Unknown serviceType in _unifiedServiceRunner: ${serviceType}`)
				finalMainResult = providerResponse
			}
