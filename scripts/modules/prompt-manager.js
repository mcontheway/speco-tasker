...
export class PromptManager {
	...
	setupValidation() {
		...
		try {
			...
			const schemaPath = path.join(this.promptsDir, 'schemas', 'prompt-template.schema.json')
	...
	validateParameters(template, variables) {
		...
		for (const [paramName, paramConfig] of Object.entries(template.parameters)) {
			...
			if (!this.validateParameterType(value, paramConfig.type)) {
				errors.push(`Parameter '${paramName}' expected ${paramConfig.type}, got ${typeof value}`)
			}
			...
			if (typeof value === 'number') {
				if (paramConfig.minimum !== undefined && value < paramConfig.minimum) {
					errors.push(`Parameter '${paramName}' must be >= ${paramConfig.minimum}`)
				}
				if (paramConfig.maximum !== undefined && value > paramConfig.maximum) {
					errors.push(`Parameter '${paramName}' must be <= ${paramConfig.maximum}`)
				}
			}
		}
	...
	selectVariant(template, variables) {
		...
		for (const [name, variant] of Object.entries(template.prompts)) {
			...
			if (variant.condition && this.evaluateCondition(variant.condition, variables)) {
				return { ...variant, name }
			}
		}
	...
	renderTemplate(template, variables) {
		...
		rendered = rendered.replace(/\{\{\{json\s+(\w+(?:\.\w+)*)\}\}\}/g, (match, path) => {
			const value = this.getNestedValue(variables, path)
	...
	validateAllPrompts() {
		...
		try {
			...
			results.errors.push(`Failed to read templates directory: ${error.message}`)
		}
