...
import { MOVE_ERROR_CODES, MoveTaskError, moveTasksBetweenTags } from './task-manager/move-task.js'
...
async function runInteractiveSetup(projectRoot) {
	if (!projectRoot) {
		console.error(chalk.red('Error: Could not determine project root for interactive setup.'))
		process.exit(1)
	}
	...
	if (!currentConfigResult.success && currentConfigResult.error?.code !== 'CONFIG_MISSING') {
		console.warn(
			chalk.yellow(
	...
	console.log(chalk.gray('   • Standard models are listed first, custom providers at bottom'))
	...
	if (answers.mainModel !== '__CANCEL__') {
		...
		answers.researchModel = await search({
			...
			source: createSearchSource(researchPromptData.choices, researchPromptData.default),
			pageSize: 15
		})
		...
		if (answers.researchModel !== '__CANCEL__') {
			...
			answers.fallbackModel = await search({
				...
				source: createSearchSource(fallbackPromptData.choices, fallbackPromptData.default),
				pageSize: 15
			})
		}
	}
	...
	async function handleSetModel(role, selectedValue, currentModelId) {
		...
		if (selectedValue === '__CUSTOM_OPENROUTER__') {
			...
			if (!openRouterModels || !openRouterModels.some((m) => m.id === modelIdToSet)) {
				console.error(
					chalk.red(
			...
			if (ollamaModels === null) {
				...
				console.log(chalk.yellow(`You can check available models with: curl ${ollamaBaseURL}/tags`))
				setupSuccess = false
				return true // Continue setup, but mark as failed
			}
			...
			console.log(
				chalk.blue(`Custom Bedrock model "${modelIdToSet}" will be used. No validation performed.`)
			)
		...
		if (modelIdToSet !== currentModelId) {
			if (modelIdToSet) {
				...
				if (result.success) {
					console.log(
						chalk.blue(`Set ${role} model: ${result.data.provider} / ${result.data.modelId}`)
					)
					...
					console.error(
						chalk.red(`Error setting ${role} model: ${result.error?.message || 'Unknown'}`)
					)
					setupSuccess = false
				}
				...
				if (currentCfg?.models?.fallback?.modelId) {
					...
					if (writeConfig(currentCfg, projectRoot)) {
						...
						console.error(chalk.red('Failed to disable fallback model in config file.'))
						setupSuccess = false
					}
				} else {
...
function registerCommands(programInstance) {
	...
	programInstance
		...
		.option('-n, --num-tasks <number>', 'Number of tasks to generate', getDefaultNumTasks())
		...
		.action(async (file, options) => {
			...
			try {
				...
				const outputPath =
					taskMaster.getTasksPath() || path.join(taskMaster.getProjectRoot(), TASKMASTER_TASKS_FILE)
	...
	programInstance
		...
		.option('-p, --prompt <text>', 'Prompt explaining the changes or new context (required)')
		.option('-r, --research', 'Use Perplexity AI for research-backed task updates')
		.option('--append', 'Append timestamped information to task details instead of full update')
		.option('--tag <tag>', 'Specify tag context for task operations')
		.action(async (options) => {
			try {
				// Initialize TaskMaster
	...
	programInstance
		...
		.option('-f, --file <file>', 'Path to the tasks file', TASKMASTER_TASKS_FILE)
		.option('-i, --id <ids>', 'Comma-separated task/subtask IDs to scope up (required)')
		.option(
			'-s, --strength <level>',
			'Complexity increase strength: light, regular, heavy',
			'regular'
		)
		.option('-p, --prompt <text>', 'Custom instructions for targeted scope adjustments')
		...
		.action(async (options) => {
			try {
				...
				if (!options.id) {
					console.error(chalk.red('Error: --id parameter is required'))
					console.log(
						chalk.yellow('Usage example: task-master scope-up --id=1,2,3 --strength=regular')
					)
					process.exit(1)
				}
				...
				console.log(chalk.blue(`Scoping up ${taskIds.length} task(s): ${taskIds.join(', ')}`))
				...
				console.log(chalk.green(`✅ Successfully scoped up ${result.updatedTasks.length} task(s)`))
			} catch (error) {
				console.error(chalk.red(`Error: ${error.message}`))
	...
	programInstance
		...
		.option('-f, --file <file>', 'Path to the tasks file', TASKMASTER_TASKS_FILE)
		.option('-i, --id <ids>', 'Comma-separated task/subtask IDs to scope down (required)')
		.option(
			'-s, --strength <level>',
		...
		.action(async (options) => {
			try {
				...
				if (!options.id) {
					console.error(chalk.red('Error: --id parameter is required'))
					console.log(
						chalk.yellow('Usage example: task-master scope-down --id=1,2,3 --strength=regular')
					)
					process.exit(1)
				}
				...
				console.log(chalk.blue(`Scoping down ${taskIds.length} task(s): ${taskIds.join(', ')}`))
				console.log(chalk.blue(`Strength level: ${options.strength}`))
				if (options.prompt) {
					console.log(chalk.blue(`Custom instructions: ${options.prompt}`))
				}
				...
				console.log(
					chalk.green(`✅ Successfully scoped down ${result.updatedTasks.length} task(s)`)
				)
			} catch (error) {
				console.error(chalk.red(`Error: ${error.message}`))
	...
	programInstance
		...
		.option('-a, --all', 'Expand all pending tasks based on complexity analysis')
		.option(
			'-n, --num <number>',
			'Number of subtasks to generate (uses complexity analysis by default if available)'
		)
		.option('-r, --research', 'Enable research-backed generation (e.g., using Perplexity)', false)
		...
		.action(async (options) => {
			...
			if (options.all) {
				...
				try {
					...
					console.error(chalk.red(`Error expanding all tasks: ${error.message}`))
					process.exit(1)
				}
			} else if (options.id) {
				// --- Handle expand --id <id> (Should be correct from previous refactor) ---
				if (!options.id) {
					console.error(chalk.red('Error: Task ID is required unless using --all.'))
					process.exit(1)
				}
				...
				try {
					...
					console.error(chalk.red(`Error expanding task ${options.id}: ${error.message}`))
					process.exit(1)
				}
			} else {
				console.error(chalk.red('Error: You must specify either a task ID (--id) or --all.'))
	...
	programInstance
		...
		.option('-c, --context <text>', 'Additional custom context to include in the research prompt')
		.option('-t, --tree', 'Include project file tree structure in the research context')
		.option('-s, --save <file>', 'Save research results to the specified task/subtask(s)')
		...
		.option('--save-file', 'Save research results to .taskmaster/docs/research/ directory')
		.option('--tag <tag>', 'Specify tag context for task operations')
		.action(async (prompt, options) => {
			...
			if (!prompt || typeof prompt !== 'string' || prompt.trim().length === 0) {
				console.error(chalk.red('Error: Research prompt is required and cannot be empty'))
				showResearchHelp()
				process.exit(1)
			}
			...
			if (options.detail && !validDetailLevels.includes(options.detail.toLowerCase())) {
				console.error(
					chalk.red(`Error: Detail level must be one of: ${validDetailLevels.join(', ')}`)
				)
				process.exit(1)
			}
			...
			if (options.files) {
				try {
					...
					console.error(chalk.red(`Error parsing file paths: ${error.message}`))
					process.exit(1)
				}
			}
			...
			if (options.saveTo) {
				...
				if (!/^\d+(\.\d+)?$/.test(saveToId)) {
					console.error(
						chalk.red('Error: Save-to ID must be in format "15" for task or "15.2" for subtask')
					)
					process.exit(1)
				}
			}
			...
			if (options.save) {
				...
				if (saveTarget.includes('..') || saveTarget.startsWith('/')) {
					console.error(chalk.red('Error: Save path must be relative and cannot contain ".."'))
					process.exit(1)
				}
			}
			...
			if (taskIds.length > 0) {
				try {
					...
					console.error(chalk.red(`Error reading tasks file: ${error.message}`))
					process.exit(1)
				}
			}
			...
			if (validatedParams.taskIds.length > 0) {
				console.log(chalk.gray(`Task context: ${validatedParams.taskIds.join(', ')}`))
			}

			if (validatedParams.filePaths.length > 0) {
				console.log(chalk.gray(`File context: ${validatedParams.filePaths.join(', ')}`))
			}
...
${result.result}`
						...
						if (isSubtask) {
							...
							console.log(chalk.green(`✅ Research saved to subtask ${validatedParams.saveToId}`))
						} else {
							// Save to task
							const updateTaskById = (await import('./task-manager/update-task-by-id.js')).default
							...
							console.log(chalk.green(`✅ Research saved to task ${validatedParams.saveToId}`))
						}
					} catch (saveError) {
						console.log(chalk.red(`❌ Error saving to task/subtask: ${saveError.message}`))
					}
				}
...
`
	...
	programInstance
		...
		.action(async (options) => {
			...
			if (dependenciesArray.length > 0) {
				console.log(chalk.blue(`Dependencies: [${dependenciesArray.join(', ')}]`))
			}
			if (options.priority) {
	...
	programInstance
		...
		.action(async (options) => {
			...
			await removeDependency(taskMaster.getTasksPath(), formattedTaskId, formattedDependencyId, {
				projectRoot: taskMaster.getProjectRoot(),
				tag
			})
		})
	...
	programInstance
		...
		.option('-f, --file <file>', 'Path to the report file', COMPLEXITY_REPORT_FILE)
		.option('--tag <tag>', 'Specify tag context for task operations')
		.action(async (options) => {
			const initOptions = {
				tag: options.tag
	...
	programInstance
		...
		.action(async (options) => {
			...
			if (!parentId) {
				console.error(
					chalk.red('Error: --parent parameter is required. Please provide a parent task ID.')
				)
				showAddSubtaskHelp()
				process.exit(1)
			}
			...
			try {
				if (existingTaskId) {
					...
					console.log(
						chalk.blue(`Converting task ${existingTaskId} to a subtask of ${parentId}...`)
					)
					await addSubtask(
						taskMaster.getTasksPath(),
						parentId,
		...
		.action(async (options) => {
			...
			try {
				...
				for (const subtaskId of subtaskIdArray) {
					...
					if (!subtaskId.includes('.')) {
						console.error(
							chalk.red(`Error: Subtask ID "${subtaskId}" must be in format "parentId.subtaskId"`)
						)
						showRemoveSubtaskHelp()
						process.exit(1)
					}
	...
	programInstance
		...
		.action(async (options) => {
			...
			if (!taskIdsString) {
				...
				console.error(chalk.yellow('Usage: task-master remove-task --id=<taskId1,taskId2...>'))
				process.exit(1)
			}
			...
			try {
						...
						if (taskObject) {
							...
							if (!taskObject.isSubtask) {
								...
								const dependentTasks = data.tasks.filter(
									(t) => t.dependencies && t.dependencies.includes(parseInt(taskId, 10))
								)
				...
				if (!options.yes) {
					...
					existingTasksToRemove.forEach(({ id, task }) => {
						...
						if (task.isSubtask) {
							...
							console.log(chalk.white(`  Subtask ${id}: ${task.title || '(no title)'}`))
							...
							console.log(chalk.white.bold(`  Task ${id}: ${task.title || '(no title)'}`))
						}
					})
					...
					if (dependentTaskMessages.length > 0) {
						console.log(
							chalk.yellow('⚠️ Warning: Dependencies on the following tasks will be removed:')
						)
						dependentTaskMessages.forEach((msg) => console.log(chalk.yellow(msg)))
					}
				...
				const existingIdsString = existingTasksToRemove.map(({ id }) => id).join(',')
	...
	programInstance
		...
		.option('--set-main <model_id>', 'Set the primary model for task generation/updates')
		.option('--set-research <model_id>', 'Set the model for research-backed operations')
		.option('--set-fallback <model_id>', 'Set the model to use if the primary fails')
		.option('--setup', 'Run interactive setup to configure models')
		.option('--openrouter', 'Allow setting a custom OpenRouter model ID (use with --set-*) ')
		.option('--ollama', 'Allow setting a custom Ollama model ID (use with --set-*) ')
		.option('--bedrock', 'Allow setting a custom Bedrock model ID (use with --set-*) ')
		.option('--claude-code', 'Allow setting a Claude Code model ID (use with --set-*)')
		.option('--azure', 'Allow setting a custom Azure OpenAI model ID (use with --set-*) ')
		.option('--vertex', 'Allow setting a custom Vertex AI model ID (use with --set-*) ')
		.option('--gemini-cli', 'Allow setting a Gemini CLI model ID (use with --set-*)')
		.addHelpText(
			'after',
			`
Examples:
  ...
  $ task-master models --setup                            # Run interactive setup`
		...
		.action(async (options) => {
			...
			const isSetOperation = options.setMain || options.setResearch || options.setFallback
			...
			if (isSetOperation) {
				...
				if (options.setMain) {
					...
					if (result.success) {
						...
						if (result.data.warning) console.log(chalk.yellow(result.data.warning))
						updateOccurred = true
					} else {
						console.error(chalk.red(`❌ Error setting main model: ${result.error.message}`))
					}
				}
				if (options.setResearch) {
					...
					if (result.success) {
						...
						console.error(chalk.red(`❌ Error setting research model: ${result.error.message}`))
					}
				}
				if (options.setFallback) {
					...
					if (result.success) {
						...
						console.error(chalk.red(`❌ Error setting fallback model: ${result.error.message}`))
					}
				}
			...
			if (!configResult.success) {
				console.error(chalk.red(`❌ Error fetching configuration: ${configResult.error.message}`))
			} else {
				displayModelConfiguration(configResult.data, availableResult.data?.models || [])
			}
	...
	programInstance
		...
		.action(async (options) => {
			...
			async function handleCrossTagMove(moveContext, options) {
				...
				if (!sourceId) {
					console.error(chalk.red('Error: --from parameter is required for cross-tag moves'))
					showMoveHelp()
					process.exit(1)
				}
				...
				console.log(
					chalk.blue(`Moving tasks ${sourceIds.join(', ')} from "${sourceTag}" to "${toTag}"...`)
				)
			...
			async function handleWithinTagMove(moveContext) {
				...
				if (!sourceId || !destinationId) {
					console.error(
						chalk.red('Error: Both --from and --to parameters are required for within-tag moves')
					)
					console.log(
				...
				if (sourceIds.length !== destinationIds.length) {
					console.error(chalk.red('Error: The number of source and destination IDs must match'))
					console.log(chalk.yellow('Example: task-master move --from=5,6,7 --to=10,11,12'))
					process.exit(1)
				}
				...
				if (sourceIds.length > 1) {
					...
					if (!tasksData || !tasksData.tasks) {
						console.error(
							chalk.red(`Error: Invalid or missing tasks file at ${taskMaster.getTasksPath()}`)
						)
						process.exit(1)
					}
					...
					for (let i = 0; i < sourceIds.length; i++) {
						...
						try {
							await moveTask(taskMaster.getTasksPath(), fromId, toId, i === sourceIds.length - 1, {
								projectRoot: taskMaster.getProjectRoot(),
								tag
							})
							console.log(chalk.green(`✓ Successfully moved task/subtask ${fromId} to ${toId}`))
					...
					if (moveErrors.length > 0) {
						...
						console.log(chalk.green(`✓ Successfully moved: ${successfulMoves.length} tasks`))
						console.log(chalk.red(`✗ Failed to move: ${moveErrors.length} tasks`))
					...
					console.log(chalk.blue(`Moving task/subtask ${sourceId} to ${destinationId}...`))

					const result = await moveTask(taskMaster.getTasksPath(), sourceId, destinationId, true, {
						projectRoot: taskMaster.getProjectRoot(),
						tag
					})
					console.log(
						chalk.green(`✓ Successfully moved task/subtask ${sourceId} to ${destinationId}`)
					)
				}
			}
			...
			function handleMoveError(error, moveContext) {
				...
				if (error.code === 'CROSS_TAG_DEPENDENCY_CONFLICTS') {
					...
					// Use structured error data
					const taskId = error.data.taskId || moveContext.sourceId?.split(',')[0]
					displaySubtaskMoveError(taskId, moveContext.sourceTag, moveContext.toTag)
				} else if (
		...
		.action(async (tagName, options) => {
			try {
				...
				if (!tagName && !options.fromBranch) {
					console.error(
						chalk.red('Error: Either tagName argument or --from-branch option is required.')
					)
				...
				if (options.fromBranch) {
					const { createTagFromBranch } = await import('./task-manager/tag-management.js')
					const gitUtils = await import('./utils/git-utils.js')

					// Check if we're in a git repository
					if (!(await gitUtils.isGitRepository(context.projectRoot))) {
						console.error(
							chalk.red('Error: Not in a git repository. Cannot use --from-branch option.')
						)
						process.exit(1)
					}
					...
					if (!currentBranch) {
						console.error(chalk.red('Error: Could not determine current git branch.'))
						process.exit(1)
					}
	...
	programInstance
		...
		.action(async (sourceName, targetName, options) => {
			try {
				...
				await copyTag(tasksPath, sourceName, targetName, copyOptions, context, 'text')
			} catch (error) {
...
function setupCLI() {
	...
	const programInstance = program
		...
		.version(() => {
			...
			try {
				...
				log('warn', 'Could not read package.json for version info in .version()')
			}
	...
	const originalHelpInformation = programInstance.helpInformation.bind(programInstance)
