		...
		for (const taskId of taskIdsToRemove) {
			...
			try {
				...
				if (typeof taskId === 'string' && taskId.includes('.')) {
					...
					if (subtaskIndex === -1) {
						throw new Error(`Subtask ${subtaskId} not found in parent task ${parentTaskId}`)
					}
					...
					results.messages.push(`Successfully removed subtask ${taskId} from tag '${tag}'`)
				}
				// Handle main task removal
					...
					results.messages.push(`Successfully removed task ${taskId} from tag '${tag}'`)
				}
			} catch (innerError) {
				...
				if (
					...
					currentTagTasks.forEach((task) => {
						if (task.dependencies) {
							task.dependencies = task.dependencies.filter((depId) => !allRemovedIds.has(depId))
						}
						if (task.subtasks) {
							task.subtasks.forEach((subtask) => {
								if (subtask.dependencies) {
									subtask.dependencies = subtask.dependencies.filter(
										(depId) =>
											!allRemovedIds.has(`${task.id}.${depId}`) && !allRemovedIds.has(depId)
									)
								}
							})
						}
