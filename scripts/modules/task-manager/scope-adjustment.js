...
import { generateObjectService, generateTextService } from '../ai-services-unified.js'
...
async function reanalyzeTaskComplexity(task, tasksPath, context) {
	...
	try {
		...
		const complexityReportPath = findComplexityReportPath(null, { projectRoot, tag }, null)
		...
		if (updatedReport) {
			...
			if (taskAnalysis) {
				log('info', `Re-analyzed task ${task.id} complexity: ${taskAnalysis.complexityScore}/10`)
				return taskAnalysis.complexityScore
			}
		}

		log('warn', `Could not find updated complexity analysis for task ${task.id}`)
		return null
	} catch (error) {
...
async function regenerateSubtasksForComplexity(
	...
	if (direction === 'up') {
		...
		if (strength === 'light') {
			const base = Math.max(5, preservedCount + Math.ceil(currentPendingCount * 1.1))
			targetSubtaskCount = Math.ceil(base * (0.8 + 0.4 * complexityFactor))
		} else if (strength === 'regular') {
			const base = Math.max(6, preservedCount + Math.ceil(currentPendingCount * 1.3))
			targetSubtaskCount = Math.ceil(base * (0.8 + 0.4 * complexityFactor))
		} else {
			// heavy
			const base = Math.max(8, preservedCount + Math.ceil(currentPendingCount * 1.6))
		...
		const aggressiveFactor = originalComplexity >= 8 ? 0.7 : originalComplexity >= 6 ? 0.85 : 1.0

		if (strength === 'light') {
			const base = Math.max(3, preservedCount + Math.ceil(currentPendingCount * 0.8))
			targetSubtaskCount = Math.ceil(base * aggressiveFactor)
		} else if (strength === 'regular') {
			const base = Math.max(3, preservedCount + Math.ceil(currentPendingCount * 0.5))
			targetSubtaskCount = Math.ceil(base * aggressiveFactor)
		} else {
			// heavy
			...
			const base = Math.max(2, preservedCount + Math.ceil(currentPendingCount * 0.25))
...
Ensure the JSON is valid and properly formatted.`
		...
		log('warn', `Failed to regenerate subtasks for task ${task.id}: ${error.message}`)
...
function generateScopePrompt(task, direction, strength, customPrompt) {
	...
	const strengthDescriptions = {
		...
		regular: isUp ? 'moderate complexity increases' : 'moderate simplifications',
 ...
 */
async function adjustTaskComplexity(task, direction, strength, customPrompt, context) {
	...
	const taskSchema = z.object({
		title: z.string().min(1).describe('Updated task title reflecting scope adjustment'),
		description: z.string().min(1).describe('Updated task description with adjusted scope'),
		details: z.string().min(1).describe('Updated implementation details with adjusted complexity'),
		testStrategy: z.string().min(1).describe('Updated testing approach for the adjusted scope'),
...
export async function scopeUpTask(
	...
	for (const taskId of taskIds) {
		...
		const adjustResult = await adjustTaskComplexity(task, 'up', strength, customPrompt, context)
		...
		if (context.session && originalComplexity) {
			try {
				...
				if (newComplexity && outputFormat === 'text') {
					...
					const arrow = complexityChange > 0 ? '↗️' : complexityChange < 0 ? '↘️' : '➡️'
					log(
						'info',
		...
		if (adjustResult.telemetryData) {
			if (!combinedTelemetryData) {
				...
				combinedTelemetryData.inputTokens += adjustResult.telemetryData.inputTokens || 0
				combinedTelemetryData.outputTokens += adjustResult.telemetryData.outputTokens || 0
				combinedTelemetryData.totalTokens += adjustResult.telemetryData.totalTokens || 0
				combinedTelemetryData.totalCost += adjustResult.telemetryData.totalCost || 0
			}
		}
	}
...
export async function scopeDownTask(
	...
	for (const taskId of taskIds) {
		...
		const adjustResult = await adjustTaskComplexity(task, 'down', strength, customPrompt, context)
