...
import { LEGACY_CONFIG_FILE, TASKMASTER_CONFIG_FILE } from '../../../src/constants/paths.js'
...
export async function migrateProject(options = {}) {
	...
	if (fs.existsSync(taskmasterDir) && !options.force) {
		log.warn('.taskmaster directory already exists. Use --force to overwrite or skip migration.')
		return
	}
	...
	if (migrationPlan.length === 0) {
		log.info('No files to migrate. Project may already be using the new structure.')
		return
	}
	...
	if (options.dryRun) {
		log.info('Dry run complete. Use --dry-run=false to perform actual migration.')
		return
	}
	...
	try {
		...
		if (!options.cleanup) {
			log.info('Old files were preserved. Use --cleanup to remove them after verification.')
		}
	} catch (error) {
...
async function performMigration(projectRoot, migrationPlan, options) {
	...
	for (const item of migrationPlan) {
		...
		// Create backup if requested
		if (options.backup) {
			const backupPath = path.join(projectRoot, '.taskmaster-migration-backup', item.from)
