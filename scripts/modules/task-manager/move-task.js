...
import { log, readJSON, setTasksForTag, traverseDependencies, writeJSON } from '../utils.js'
...
function normalizeDependencies(deps) {
	...
	return deps.map((d) => normalizeDependency(d)).filter((n) => Number.isFinite(n))
}
 ...
 */
async function moveTask(tasksPath, sourceId, destinationId, generateFiles = false, options = {}) {
	...
	log('info', `Moving task/subtask ${sourceId} to ${destinationId} (tag: ${tag})`)
...
function moveSubtaskToSubtask(tasks, sourceId, destinationId) {
	// Parse IDs
	const [sourceParentId, sourceSubtaskId] = sourceId.split('.').map((id) => parseInt(id, 10))
	const [destParentId, destSubtaskId] = destinationId.split('.').map((id) => parseInt(id, 10))
	...
	// Find source subtask
	const sourceSubtaskIndex = sourceParentTask.subtasks.findIndex((st) => st.id === sourceSubtaskId)
	...
	if (sourceParentId === destParentId) {
		...
		if (destParentTask.subtasks.length > 0) {
			const destSubtaskIndex = destParentTask.subtasks.findIndex((st) => st.id === destSubtaskId)
			if (destSubtaskIndex !== -1) {
				...
				const adjustedIndex =
					sourceSubtaskIndex < destSubtaskIndex ? destSubtaskIndex - 1 : destSubtaskIndex
...
function moveTaskToSubtask(tasks, sourceId, destinationId) {
	...
	if (destParentTask.subtasks.length > 0) {
		destSubtaskIndex = destParentTask.subtasks.findIndex((st) => st.id === destSubtaskId)
		if (destSubtaskIndex === -1) {
...
function moveSubtaskToAnotherParent(
	...
	let destSubtaskIndex = -1
	if (destParentTask.subtasks.length > 0) {
		destSubtaskIndex = destParentTask.subtasks.findIndex((st) => st.id === destSubtaskId_num)
		if (destSubtaskIndex === -1) {
			// Subtask doesn't exist, we'll insert at the end
			destSubtaskIndex = destParentTask.subtasks.length - 1
		}
	...
	tasks.forEach((task) => {
		...
		if (task.subtasks) {
			task.subtasks.forEach((subtask) => {
				if (subtask.dependencies && subtask.dependencies.includes(sourceTask.id)) {
...
async function validateMove(tasksPath, taskIds, sourceTag, targetTag, context) {
	...
	if (!rawData || !rawData[sourceTag] || !Array.isArray(rawData[sourceTag].tasks)) {
		throw new MoveTaskError(
 ...
 */
async function resolveDependencies(sourceTasks, allTasks, options, taskIds, sourceTag, targetTag) {
	...
	if (withDependencies) {
		...
		const allDependentTaskIdsRaw = findAllDependenciesRecursively(sourceTasks, tasksInSourceTag, {
			maxDepth: 100,
			includeSelf: false
		})
	...
	const crossTagDependencies = findCrossTagDependencies(sourceTasks, sourceTag, targetTag, allTasks)
		...
		if (ignoreDependencies) {
			...
			sourceTasks.forEach((task) => {
				...
				task.dependencies = task.dependencies.filter((depId) => {
					...
					if (Number.isFinite(parentTaskId) && sourceTagTasks.some((t) => t.id === parentTaskId)) {
						return false
					}

					// If dependency resolves to a task in the target tag, keep it
					if (Number.isFinite(parentTaskId) && targetTagTasks.some((t) => t.id === parentTaskId)) {
						return true
					}
			...
			log('warn', `Removed ${crossTagDependencies.length} cross-tag dependencies`)
...
async function executeMoveOperation(
	...
	for (const taskId of tasksToMove) {
		...
		const sourceTaskIndex = rawData[sourceTag].tasks.findIndex((t) => t.id === normalizedTaskId)
		...
		const existingTaskIndex = rawData[targetTag].tasks.findIndex((t) => t.id === normalizedTaskId)
		...
		const taskWithPreservedMetadata = preserveTaskMetadata(taskToMove, sourceTag, targetTag)
...
async function moveTasksBetweenTags(
	...
	// 1. Validation phase
	const validation = await validateMove(tasksPath, taskIds, sourceTag, targetTag, context)
...
function detectIdConflicts(taskIds, targetTag, rawData) {
	...
	taskIds.forEach((taskId) => {
		// Normalize taskId to number for comparison
		const normalizedTaskId = typeof taskId === 'string' ? parseInt(taskId, 10) : taskId
		const existingTask = rawData[targetTag].tasks.find((t) => t.id === normalizedTaskId)
		if (existingTask) {
