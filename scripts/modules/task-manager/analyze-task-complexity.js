...
import { COMPLEXITY_REPORT_FILE, LEGACY_TASKS_FILE } from '../../../src/constants/paths.js'
import { CUSTOM_PROVIDERS } from '../../../src/constants/providers.js'
import { resolveComplexityReportOutputPath } from '../../../src/utils/path-utils.js'
import { getDebugFlag, getProjectName, hasCodebaseAnalysis } from '../config-manager.js'
 ...
 */
function generateInternalComplexityAnalysisPrompt(tasksData, gatheredContext = '') {
	...
	try {
		...
		if (options._filteredTasksData) {
			...
			if (specificIds && specificIds.length > 0) {
				...
				if (outputFormat === 'text') {
					if (filteredTasks.length === 0 && specificIds.length > 0) {
						console.log(
							chalk.yellow(`Warning: No active tasks found with IDs: ${specificIds.join(', ')}`)
						)
			...
			// Apply range filtering if specified
				...
				const effectiveToId =
					toId !== null ? toId : Math.max(...originalData.tasks.map((t) => t.id))
		...
		if (originalData && originalData.tasks.length > 0) {
			try {
				...
				const fuzzySearch = new FuzzyTaskSearch(allTasksFlat, 'analyze-complexity')
				// Create a query from the tasks being analyzed
				const searchQuery = tasksData.tasks.map((t) => `${t.title} ${t.description}`).join(' ')
				...
				reportLog(`Could not gather additional context: ${contextError.message}`, 'warn')
			}
		}
		...
		if (tasksData.tasks.length === 0) {
			...
			if (existingReport && (specificIds || fromId !== null || toId !== null)) {
				...
				if (outputFormat === 'text') {
					console.log(
						chalk.yellow('No matching tasks found for analysis. Keeping existing report.')
					)
				}
				return {
			...
			fs.writeFileSync(outputPath, JSON.stringify(emptyReport, null, '\t'), 'utf8')
		...
		try {
			...
			if (outputFormat === 'text') {
				...
				console.log(chalk.green('AI service call complete. Parsing response...'))
			}
			...
			try {
				...
				if (codeBlockMatch) {
					...
					if (firstBracket !== -1 && lastBracket > firstBracket) {
						...
						reportLog('Warning: Response does not appear to be a JSON array.', 'warn')
					}
				}
				...
				if (outputFormat === 'text') {
					console.error(chalk.red(`Error parsing complexity analysis JSON: ${parseError.message}`))
				}
				throw parseError
			}
			...
			if (existingReport && Array.isArray(existingReport.complexityAnalysis)) {
				...
				const existingEntriesNotAnalyzed = existingReport.complexityAnalysis.filter(
					(item) => !analyzedTaskIds.has(item.taskId) && currentTagTaskIds.has(item.taskId) // Only keep entries for tasks in current tag
				)
			...
			if (outputFormat === 'text') {
				...
				if (getDebugFlag(session)) {
					console.debug(chalk.gray(`Final analysis object: ${JSON.stringify(report, null, 2)}`))
				}
			...
			if (outputFormat === 'text') {
				console.error(chalk.red(`Error during AI service call: ${aiError.message}`))
				if (aiError.message.includes('API key')) {
					...
					console.log(chalk.yellow("Run 'task-master models --setup' if needed."))
				}
			}
			throw aiError
		}
