...
function parseUpdatedTaskFromText(text, expectedTaskId, logFn, isMCP) {
	...
	report('info', 'Attempting to parse updated task object from text response...')
	...
	if (firstBraceIndex !== -1 && lastBraceIndex > firstBraceIndex) {
		potentialJsonFromBraces = cleanedResponse.substring(firstBraceIndex, lastBraceIndex + 1)
	...
	if (parseMethodUsed === 'raw') {
		const codeBlockMatch = cleanedResponse.match(/```(?:json|javascript)?\s*([\s\S]*?)\s*```/i)
		if (codeBlockMatch) {
	...
	try {
		...
		throw new Error(`Failed to parse JSON response object: ${parseError.message}`)
	}

	if (!parsedTask || typeof parsedTask !== 'object') {
		report('error', `Parsed content is not an object. Type: ${typeof parsedTask}`)
		report('error', `Parsed content sample: ${JSON.stringify(parsedTask).substring(0, 200)}`)
	...
	const preprocessedTask = {
		...
		dependencies: Array.isArray(parsedTask.dependencies) ? parsedTask.dependencies : [],
		details:
...
async function updateTaskById(
	...
	try {
		...
		if (taskToUpdate.status === 'done' || taskToUpdate.status === 'completed') {
			report('warn', `Task ${taskId} is already marked as done and cannot be updated`)
