...
async function monitorMemoryUsage(command, args = []) {
	return new Promise((resolve, reject) => {
		...
		console.log(
			`🚀 Starting performance monitoring for: ${command} ${args.join(" ")}`,
		);
			...
			const stats = {
				...
				memory: {
					average: {
						...
						rss: (
							rssValues.reduce((a, b) => a + b, 0) / rssValues.length
						).toFixed(2),
					},
					peak: {
					...
					growth:
						heapUsages.length > 1
							? (heapUsages[heapUsages.length - 1] - heapUsages[0]).toFixed(2)
							: "0",
				},
				exitCode: code,
			};
...
async function runBenchmark() {
	...
	try {
		...
		console.log("\n✅ Performance Assertions:");
		console.log(
			`Average heap memory < 100MB: ${avgHeap < 100 ? "PASS" : "FAIL"} (${avgHeap}MB)`,
		);
		console.log(
			`Peak heap memory < 200MB: ${peakHeap < 200 ? "PASS" : "FAIL"} (${peakHeap}MB)`,
		);
		console.log(
			`Memory growth < 10MB: ${growth < 10 ? "PASS" : "FAIL"} (${growth}MB)`,
		);

		// Test version command
		console.log("\n🔍 Testing CLI version command...");
		const versionStats = await monitorMemoryUsage("node", [
			CLI_PATH,
			"--version",
		]);
